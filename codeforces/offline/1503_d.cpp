...
struct SolverContext {
  ...
  bool is_ok(std::vector<pii> &tb) const {
    std::sort(ALL(tb));
    REP (i, tb.size() - 1)
      if (tb[i + 1].ST > tb[i].ST || tb[i + 1].ND < tb[i].ND) return false;
    return true;
  }
  ...
  OutT f(int l, int r) {
    ...
    int na = is_a[l];
    int u = peer[l];
    std::vector<pii> la = { { l, peer[l] } };
    std::vector<pii> ra = {};
    std::set<int> used = { l, peer[l] };
    ...
    while (true) {
      ra.clear();
      FOR (i, la.back().ND + 1, nr + 1)
        if (!used.count(i)) {
          ra.emplace_back(peer[i], i), na += is_a[peer[i]];
          used.insert(peer[i]);
          if (peer[i] >= la.back().ND) break;
          used.insert(i);
        }
      nr = la.back().ND - 1;

      OPA_DISP0(la, ra, nl, nr);
      if (!is_ok(ra)) break;
      if (!ra.size()) {
        ok = true;
        break;
      ...
      la.clear();
      FOR (i, nl + 1, ra[0].first)
        if (!used.count(i)) {
          la.emplace_back(i, peer[i]), na += is_a[i];
          if (peer[i] <= ra[0].first) break;
          used.insert(i);
          used.insert(peer[i]);
        }
      if (!is_ok(la)) break;
    ...
    int v = 0;
    OPA_DISP0(l, r, nl, nr);
    if (ok && nl < nr) v = f(nl, nr);
    if (v == -1) ok = false;
    if (ok) res = std::min(na, r - l - (nr - nl) - na) + v;

    dp[MP(l, r)] = res;
    return res;
  }

  OutT solve() { return f(0, 2 * n - 1); }
