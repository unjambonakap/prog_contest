...
struct SolverContext {
  ...
  vi solve() {
    ...
    vi prev_decomp(n + 1, -1);
    std::vector<u64> hl(n + 1);
    std::vector<u64> hvals(n + 1);
    hl[1] = 0;
    s64 hfact = 0;
    s64 vx = 0;
    FOR (i, 2, n + 1) {
      if (prev_decomp[i] == -1) {
        hl[i] = rng_64();
        for (s64 j = 1ull * i * i; j <= n; j += i) prev_decomp[j] = i;
      } else
        hl[i] = hl[prev_decomp[i]] ^ hl[i / prev_decomp[i]];
