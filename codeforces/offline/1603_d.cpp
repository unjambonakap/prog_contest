...
struct Precomp {
  ...
  State dr(const State &s) {

    State ns = s;
    int pos = 0;

    ns.r++;
    for (; divisors[ns.r][pos] < s.l; ++pos)
      ;

    ns.v += divisors[ns.r].size() - pos;
    OPA_CHECK(ns.v >= 0, ns.l, ns.r, s.v, pos, divisors[ns.r]);
  ...
  Precomp() {
    ...
    FOR (i, 2, maxn) {
      if (prev[i] != -1) {
        ...
        while (x != 1) {
          ...
          vi ndivs;
          for (auto v1 : divs)
            for (auto v2 : pvs) ndivs.pb(v1 * v2);
          std::swap(divs, ndivs);
        }
        std::sort(ALL(divs));
        divisors[i] = divs;
      ...
      for (s64 j = 1ll * i * i; j < maxn; j += i)
        if (prev[j] == -1) prev[j] = i;
    }
    ...
    FOR (k, 2, maxk) {
      dp[k][k] = k;
      s64 last = k - 1;
      State s = { .l = k - 1, .r = k, .v = 1 };
      FOR (n, k + 1, maxn) {
        s = dr(s);

        while (s.l != s.r) {
          auto ns = dl(s);
          if (ns.v + dp[k - 1][ns.l] <= s.v + dp[k - 1][s.l])
            s = ns;
          else
            break;
        }
        dp[k][s.r] = s.v + dp[k - 1][s.l];
      }
    }
  }
...
struct SolverContext {
  ...
  OutT solve() { return k >= maxk ? k : g_precomp.dp[k][n]; }
