...
struct Data {
  int val = -1;
  void push(const Data &peer, int dn = 0) { checkmax(val, peer.val); }
  void reset() { val = -1; }
};
...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      g_reader.readline();
      tb.resize(n);
      REP (i, n) g_reader.iss >> tb[i], --tb[i];
    }
  }

  int normv(int a, int f) { return f ? n - 1 - a : a; }

  OutT solve() {
    vi tvals[2];
    REP (f, 2) {
      REP (i, n) tvals[f].pb(n);
    }
    int cnt = 1;
    while (cnt < n) {
      ++cnt;
      ...
      SegTree st2[2];
      vi ntvals[2];
      REP (f, 2) st[f].build(n + 1);
      REP (f, 2) st2[f].build(n + 1);
      REP (f, 2) ntvals[f].resize(n, 0);
      OPA_DISP0(cnt, tvals[0]);
      OPA_DISP0(cnt, tvals[1]);
      REPV (i, n) {
        int ni = i - cnt + 1;
        REP (f, 2) st2[f ^ 1].root->add1(tvals[f][i], Data{ .val = normv(tb[i], f) });
        if (1 && ni >= 0) {
          REP (f, 2) {

            auto qr = st2[f].root->query(normv(tb[i], f), n);
            if (qr.val > normv(tb[i], f^1)) checkmax(ntvals[f][ni], qr.val);
          }
        }
        REP (f, 2) {
          auto qr = st[f].root->query(normv(tb[i], f), n);
          if (qr.val > normv(tb[i], f)) checkmax(ntvals[f][i], qr.val);
          st[f].root->add1(normv(tb[i], f), Data{ .val = tvals[f][i] });
        }
      }

      REP (f, 2) tvals[f] = ntvals[f];
      REP (f, 2)
        REP (i, n)
          if (tvals[f][i] != 0) goto found;
