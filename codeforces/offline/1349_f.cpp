...
s64 faste(int a, int p) {
  s64 x = 1;
  for (; p; p >>= 1, a = 1ll * a * a % mod)
    if (p & 1) x = 1ll * x * a % mod;
  return x;
}
s64 inv(int a) { return faste(a, mod - 2); }
...
struct Precomp {
  ...
  Precomp() {
    fact[0] = 1;
    FOR (i, 1, maxn) fact[i] = 1ull * fact[i - 1] * i % mod;

    REP (i, maxn) ifact[i] = inv(fact[i]);
  }

  int ank(int n, int k) {
    if (n < 0 || k < 0 || k > n) return 0;
    return 1ull * fact[n] * ifact[n-k] % mod;
  }
  int cnk(int n, int k) {
    if (n < 0 || k < 0 || k > n) return 0;
...
struct SolverContext {
  ...
  s64 snk(int k) {
    s64 res = 0;
    REP (u, n - k + 1) {
      res += faste(k, u) * faste(k - 1, n - u - k) % mod * g_precomp.cnk(n - u - 1, k - 1) % mod;
      OPA_DISP0(res, u);
    }
    return res % mod;
  }
  ...
  OutT solve() {
    std::vector<vi> dp(n + 1, vi(n + 1, 0));
    dp[1][0] = 1;
    FOR (i, 1, n) {
      REP (j, i + 1)
        dp[i + 1][j] =
          1ull * dp[i][j] * (j + 1) + (j == 0 ? 0 : 1ull * dp[i][j - 1] * (i + 1 - j)) % mod;
    }
    vi res(n);
    OPA_DISP0(n,dp[n]);
    REP (i, n) {
      REP (k, i + 1) res[k] = (res[k] + 1ull * dp[i + 1][k] * g_precomp.ank(n, n-i-1)) % mod;
    }
    return res;
  }
  ...
  OutT solve_dumb() {
    ...
    for (auto x : lst) {
      std::map<int, pii> vals;
      REP (i, n) vals[i] = MP(n, -1);
      REP (i, n) {
        checkmin(vals[x[i]].ST, i);
        checkmax(vals[x[i]].ND, i);
      }
      bool ok = true;
      for (auto e : vals) {
        int v = e.first;
        if (e.second.second == -1) continue;
        if (v == 0) continue;
        if (vals[v - 1].first > e.second.second) {
          ok = false;
          break;
        }
      }
