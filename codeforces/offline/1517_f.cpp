...
struct SolverContext {
  ...
  Data proc(int pos, int maxr, int par = -1) {
    std::vector<Data> children;
    for (auto b : edges[pos])
      if (b != par) children.pb(proc(b, maxr, pos));
    Data res;
    REP (f, 2) res.data[f].resize(maxr + 2, 0);

    REP (i, maxr + 1) {
      s64 v = 1;
      if (i > 0) {
        for (auto &x : children) v = v * (x.data[0][i + 1] + x.data[i == 1 ? 0 : 1][i - 1] + x.data[1][i]) % mod;
        res.data[0][i] = v;
      }
      ...
      for (auto &x : children) {
        ...
        s64 down = x.data[i <= 1 ? 0 : 1][std::max(0, i - 1)];
        s64 up = x.data[0][i + 1];
        v1 = v1 * (down + up) % mod;
        v2 = v2 * up % mod;
      }
      if (i > 0)
        res.data[1][i] += (mod + v1 - v2) % mod;
      else
        res.data[0][i] = v1;
    }
  ...
  OutT solve() {
    ...
    REP (maxr, n + 1) {
      auto d = proc(0, maxr);
      s64 u = d.data[0][0];
      REP (i, maxr + 1) u += d.data[1][i];
      u %= mod;
      s64 cur = u - last;
      s64 level = maxr == 0 ? negp(1) : maxr - 1;
      res = (res + level * cur) % mod;
      last = u;
    }
    res += n;
    return res % mod * inv(tot) % mod;
  }
