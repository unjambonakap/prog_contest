...
inline u64 low_bit(u64 n){return (n^n-1)&n;}
inline int ctz(u64 n){return (n==0?-1:ctz(n>>1)+1);}
...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      REP (i, k) {
        ...
        vi u(nx);
        REP (j, nx) g_reader.iss >> u[j], --u[j];
        ops.pb(u);
      }
    }
  }

  bool rec(const State &state, int step) {
    if (step == ops.size()) {
      if (state.mask != (1ull << n) - 1) return false;
      if (state.v == 0) return true;
      if (state.v != state.mask - low_bit(state.v) +1) return false;
      return true;
    }

    int rank = -1;
    const auto &s = ops[step];

    int sz = s.size();
    u64 smask = 0;
    REP (i, sz) smask |= 1ull << s[i];
    int n1 = count_bit(smask & state.mask & state.v);
    int n0 = count_bit(smask & state.mask & ~state.v);
    ...
    REP (nn1, sz + 1) {
      if (nn1 >= n1 && sz - nn1 >= n0) {
        if (!rec(ns, step + 1)) return false;
      }
      if (nn1 < sz) ns.v |= 1ull << s[sz - 1 - nn1];
    }
  ...
  bool checkit(int x) {
    State s = State{ .mask = 0, .v = 0 };
    return rec(s, 0);
  }

  OutT solve() {
    REP (i, n)
      if (!checkit(i)) return false;
    return true;
  }
