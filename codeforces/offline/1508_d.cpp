...
template <class T> struct Segment {
  ...
  bool intersects(const Segment &peer, bool strict) const {
    if (parallel(peer)) return false;
    if (!strict) {

      if (a == peer.a || a == peer.b) return false;
      if (b == peer.a || b == peer.b) return false;
    }
...
typedef std::pair<bool, std::vector<pii> > OutT;
struct Data {

  int a, x, y;
  Point<s64> p() const { return P64(x, y); }
};
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      data.resize(n);
      REP (i, n) {
        g_reader.readline() >> data[i].x >> data[i].y >> data[i].a;
        --data[i].a;
      }
    }
  }
  ...
  OutT solve() {
    ...
    if (sel == -1) return MP(true, sol);

    vi perm(n);
    REP (i, n) perm[i] = data[i].a;
    std::vector<int> order;
    REP (i, n)
      if (i != sel) order.pb(i);
    auto cx = data[sel].p();
    std::sort(ALL(order), [&](int a, int b) {
      auto &da = data[a];
      auto &db = data[b];
      auto va = da.p() - cx;
      auto vb = db.p() - cx;
      if ((va.x >= 0) != (vb.x >= 0)) return va.x >= 0;
      if (va.x == 0 && vb.x == 0) return va.y >= vb.y;
    ...
    int x = sel;
    REP (i, n - 1) {
      sol.emplace_back(sel, perm[x]);
      x = perm[x];
    }
    REP(i,n) perm[i] = data[i].a;
    for (auto x: sol){
      std::swap(perm[x.first], perm[x.second]);
    }
    REP(i,n) OPA_CHECK0(perm[i] == i);

    REP (i, sol.size())
      REP (j, i)
        OPA_CHECK0(
          !Segment<s64>(data[sol[i].first].p(), data[sol[i].second].p())
             .intersects(Segment<s64>(data[sol[j].first].p(), data[sol[j].second].p()), false));

    return MP(true, sol);
  }

  OutT solve_dumb() { return {}; }
};
  ...
  REP (ti, tn) {
    ...
    if (TEST_CHECK) {
      ...
      if (!res.first)
        ...
        printf("%lu\n", res.second.size());
        for (auto x : res.second) printf("%d %d\n", x.first + 1, x.second + 1);
      }
    }
  }
