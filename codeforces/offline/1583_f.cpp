...
struct SolverContext {
  ...
  std::pair<int, vi> solve() {
    ...
    std::vector<vi> states(n, vi(nc));
    REPV (i, n) {
      vi state;
      int x = n - 1 - i;
      REP (j, nc) state.pb(x % k), x /= k;
      states[i] = state;
      FOR (j, i + 1, n) {
        int sel = -1;
        REP (u, nc)
          if (state[u] > states[j][u]) {
            sel = u;
            break;
          }
        OPA_CHECK(sel != -1, state, states[j], sel, nc);
        edges.pb(sel + 1);
      }
    }
  ...
  std::pair<int, vi> solve_dumb() { return {}; }
};
