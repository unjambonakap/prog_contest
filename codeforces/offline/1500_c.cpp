...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      REP (i, n) {
        g_reader.readline();
        REP (j, m) g_reader.iss >> a[j][i];
      }
      b.resize(n, vi(m));
      REP (i, n) {
        g_reader.readline();
        REP (j, m) g_reader.iss >> b[j][i];
      }
    }
  }

  void sort_a(vi &order, int col) {
    std::stable_sort(ALL(order), [&](int a, int b) { return this->a[col][a] < this->a[col][b]; });
  }

  vi solve() {
    vi res;
    std::vector<vi> sort_seq;
    std::set<int> rem;
    REP (i, m) rem.insert(i);

    std::vector<int> split = { 0, n };
    std::vector<vi> b_next_sorted(m, vi(n));
    REP (i, m) {
      std::vector<pii> state;
      state.emplace_back(n, -1);
      int v = n;
      REPV (j, n) {
        while (state.back().second >= b[i][j]) state.pop_back();
        checkmin(v, state.back().first);
        b_next_sorted[i][j] = v;
        state.emplace_back(j, b[i][j]);
      }
    }
    ...
    while (true) {
      ...
      REP (i, split.size() - 1) {
        ...
        while (l < h) {
          nsplit.pb(l);
          int nl = h;
          for (auto x : take) checkmin(nl, b_next_sorted[x][l]);
          l = nl;
        }
      }
      split = nsplit;
    }
    std::reverse(ALL(sort_seq));

    vi order;
    REP (i, n) order.pb(i);
    for (auto x : sort_seq) {
      for (auto y : x) res.pb(y);
    }

    for (auto y : res) sort_a(order, y);
    REP (j, m)
      REP (i, n)
        if (a[j][order[i]] != b[j][i]) return { -1 };

    return res;
  }

  vi solve_dumb() { return {}; }
};
