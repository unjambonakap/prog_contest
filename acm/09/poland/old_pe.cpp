...
int main(){
    ...
    REP(ti, tn){
        ...
        REP(i, 2){
            ...
            step=(xmax-xmin)/(nstep+1);
            if (xmin+eps<xmax) REP(j, nstep){
                ld x1=xmin+j*step, x2=x1+step/2, x3=x1+step;
                ans+=step*(
                        f(sqrt(getr(x1,i)), sqrt(getr(x1,2)), ya[2])
                        +4*f(sqrt(getr(x2,i)), sqrt(getr(x2,2)), ya[2])
                        +f(sqrt(getr(x3,i)), sqrt(getr(x3,2)), ya[2])
                        )/6;
            }
            res-=ans;
        }
        ...
        if (xmin<xmax) REP(i, nstep){
            ld x1=xmin+i*step, x2=x1+step/2, x3=x1+step;
            ans+=step*(
                    f(sqrt(min(getr(x1,0), getr(x1,1))), sqrt(getr(x1,2)), ya[2])
                    +4*f(sqrt(min(getr(x2,0), getr(x2,1))), sqrt(getr(x2,2)), ya[2])
                    +f(sqrt(min(getr(x3,0), getr(x3,1))), sqrt(getr(x3,2)), ya[2])
                    )/6;
        }
        res+=ans;
