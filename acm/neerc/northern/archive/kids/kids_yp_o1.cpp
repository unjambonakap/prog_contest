...
void relax( int i1, int j1, int k1, long long s1, int i2, int j2, int k2, long long s2 ) {
  long long t = s1 - s2;
  if (be < t) {
    be = t;
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    int pmi = 0;
    long long sqMinCur = square(i, i + 1, i + 2);
    for (int j = 2, k = j + 1; j + 1 < n; j++) {
      long long t = square(i + j - 2, i + j - 1, i + j);
      if (t < sqMinCur) {
        sqMinCur = t;
        pmi = j - 2;
      }
...
//      fprintf(stderr, "i=%d j=%d k=%d\n", i + 1, (i + j) % n + 1, (i + k) % n + 1);
      relax(i, i + j, i + k, sqMaxCur, i + pmi, i + pmi + 1, i + pmi + 2, sqMinCur);
//      relax(i, i + j, i + k, sqMaxCur, i, i + 1, i + j, square(i, i + 1, i + j));
//      relax(i, i + j, i + k, sqMaxCur, i, i + j - 1, i + j, square(i, i + j - 1, i + j));
    }
  }
  printf(LL "." LL "\n", be / 2, (be % 2) * 5);
