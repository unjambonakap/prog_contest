#include <opa_common.h>

using namespace std;
const int maxn = 555555;
const int ytarget = 0;
const int xmin = -10;
const int xmax = 1e9 + 2;
const int ysrc = 1e9 + 2;
const int yceiling = ysrc + 1e8;
const int yfloor = ytarget - 1e8;

struct Tarp;

namespace treap {
std::mt19937 gen(0);
struct Node {
  int id;
  Node *l, *r;
  Node *parent;
  int d, cost;
  Node() { id = gen(); }
  void norm() {}
};

Node *join(Node *a, Node *b) {
  if (a == nullptr) return b;
  if (b == nullptr) return a;
  if (a->id > b->id) return join(b, a);
  a->norm();
  b->norm();
  Node *r;
  if (a->d < b->d)
    r = join(a->r, b);
  else
    r = join(a->l, b);
  r->parent = a;
  return a;
}

void relink(Node *x, Node *nx) {
  auto parent = x->parent;
  nx->parent = parent;
  if (parent == nullptr) return;
  if (parent->l == x)
    parent->l = nx;
  else
    parent->r = nx;
}

void remove(Node *x) {
  x->norm();
  auto r = join(x->l, x->r);
  relink(x, r);
  x->l = x->r = nullptr;
}

Node *max(Node *x){
  if (x==nullptr) return x;
  return x->r == nullptr ? x : max(x->r);
}

Node *min(Node *x){
  if (x==nullptr) return x;
  return x->l == nullptr ? x : min(x->l);
}

Node *find(Node *x, int v){
  if (x ==nullptr)return nullptr;
  if (x->d == v) return x;

  if (x->d > v){
    auto r= find(x->l, v);
    return  r==nullptr ? x : r;
  } else {
    return find(x->r, v);
  }
}

void cut(Node *x, int splitpos, Node *&l, Node *&r) {
  if (x == nullptr) return;
  x->norm();

  if (x->d > splitpos) {
    auto ln = x->l;
    ln->parent = x->l = nullptr;
    r = join(r, x);
    cut(ln, splitpos, l, r);

  } else {
    auto rn = x->r;
    rn->parent = x->r = nullptr;
    l = join(l, x);
    cut(rn, splitpos, l, r);
  }
}

} // namespace treap

struct TarpLink {
  int xl, xh;
  int dist;
  Tarp *target = nullptr;
  bool operator<(const TarpLink &peer) const { return dist < peer.dist; }
};

struct ScoreEntry {
  int d;
  int cost;
  OPA_DECL_COUT_OPERATOR2(ScoreEntry, a.d, a.cost);
  ScoreEntry(int d, int cost) : d(d), cost(cost) {}
  bool operator<(const ScoreEntry &peer) const { return d < peer.d; }
};

struct TarpScore {

  OPA_DECL_COUT_OPERATOR2(TarpScore, a.entries);
  std::set<ScoreEntry> entries;
  int query_best() const {
    assert(entries.size() != 0);
    return entries.rbegin()->cost;
  }
  TarpScore() { add(ScoreEntry(0, maxn)); }

  int query(int d) const {
    OPA_CHECK0(d >= 0);
    auto it = entries.lower_bound(ScoreEntry(d + 1, 0));
    OPA_CHECK(it != entries.begin(), d, entries.begin()->d);
    return (--it)->cost;
  }

  void add(const ScoreEntry &entry) {
    auto it = entries.lower_bound(entry);
    auto next = it;
    if (next != entries.begin()) {
      auto prev = --it;
      if (prev->cost <= entry.cost) return;
    }
    while (next != entries.end() && next->cost > entry.cost) {
      auto cur = next++;
      entries.erase(cur);
    }
    entries.insert(entry);
  }
};

struct Tarp {
  int xl, xh, y1, y2;
  int tid;
  double c;
  bool downleft;
  bool is_src = false;

  std::vector<TarpLink> uplinks;
  std::vector<TarpLink> downlinks;
  TarpScore score;
  int deg_seen = 0;
  void setup() {
    if (xl > xh) std::swap(xl, xh), std::swap(y1, y2);
    c = 1. * (y2 - y1) / (xh - xl);
    downleft = y1 < y2;
  }
  void set(int xl, int xh, int y1, int y2) {
    this->xl = xl;
    this->xh = xh;
    this->y1 = y1;
    this->y2 = y2;
    setup();
  }

  float y(int x) const { return 1. * (x - xl) * c + y1; }
  int drill_cost(int x) const { return is_src ? 0 : 1 - falling(x); }
  bool falling(int x) const { return downleft ? x == xl : x == xh; }

  int x2dist(int x) const { return downleft ? xh - x : x - xl; }
  int dist2x(int d) const { return downleft ? xh - d : xl + d; }
  void add_score(int x1, int v) { score.add(ScoreEntry(x2dist(x1), v)); }
  int query(int x) { return score.query(x2dist(x)) + drill_cost(x); }

  void finish_links() {
    std::sort(ALL(uplinks));
    // OPA_DISP0(tid);
    // for (auto &x : uplinks) OPA_DISP0(x.target->tid, x.dist, x.xl, x.xh);
  }

  TarpLink buildlink;

  void push(int x, Tarp *t) {

    // if (t != nullptr) OPA_DISP("Push ", x, tid, t->tid);
    buildlink.xh = x;
    if (buildlink.target != nullptr) {
      TarpLink inv = buildlink;
      inv.target = this;
      buildlink.target->downlinks.pb(inv);

      // OPA_DISP0(buildlink.target->tid);
      OPA_CHECK0(buildlink.xl >= xl && buildlink.xh <= xh);
      OPA_CHECK0(buildlink.target->xl <= buildlink.xl);
      OPA_CHECK(buildlink.target->xh >= x, buildlink.target->tid, t->tid, x,
                tid);
      uplinks.pb(buildlink);
    }
    buildlink.xl = buildlink.xh;
    buildlink.target = t;
  }
};

struct Event {
  int x;
  int tid;
  bool operator<(const Event &p) const { return x < p.x; }
};

int xl, xh, n;
Tarp tarps[maxn];
Tarp *t_ceiling;
Tarp *t_floor;
Tarp *t_target;
Tarp *t_src;
int state_x;

struct State {
  int tid;
  State(int tid) : tid(tid) {}

  bool operator<(const State &peer) const {
    const auto &a = tarps[tid];
    const auto &b = tarps[peer.tid];
    bool res = false;
    if (tid == peer.tid)
      res = false;
    else if (a.xl >= b.xl && a.xl <= b.xh) {
      int xa = a.xl - b.xl;
      int ya = a.y1 - b.y1;

      int xb = b.xh - b.xl;
      int yb = b.y2 - b.y1;
      return 1ll * xb * ya - 1ll * yb * xa < 0;
    } else
      res = !(peer < *this);
    // OPA_CHECK(res == a.y(state_x) < b.y(state_x), a.xl, a.xh, a.y1, a.y2,
    // b.xl,
    //          b.xh, b.y1, b.y2);
    // return a.y(state_x) < b.y(state_x);
    return res;
  }
};

void compute(Tarp *x) {
  // puts("");
  for (auto &ul : x->uplinks) {
    int s1 = ul.target->x2dist(ul.xl);
    int s2 = ul.target->x2dist(ul.xh);
    if (s2 < s1) std::swap(s2, s1);

    int sl = ul.target->query(ul.xl);
    int sh = ul.target->query(ul.xh);
    // OPA_DISP0(x->tid, ul.target->tid, ul.target->score.entries, ul.xl, ul.xh,
    // sl, sh);
    x->add_score(ul.xl, sl);
    x->add_score(ul.xh, sh);

    if (!ul.target->is_src && ul.target->downleft != x->downleft) {
      continue;
    }

    auto it1 = ul.target->score.entries.upper_bound(ScoreEntry(s1 - 1, 0));
    auto it2 = ul.target->score.entries.lower_bound(ScoreEntry(s2 + 1, 0));
    while (it1 != it2) {
      int xa = ul.target->dist2x(it1->d);
      // OPA_DISP0("cnd ", xa, it1->cost);
      x->add_score(xa, it1->cost + ul.target->drill_cost(xa));
      ++it1;
    }
  }
  // OPA_DISP0(x->tid, x->score);
}

int main(int argc, char **argv) {
  opa::init::opa_init(argc, argv);
  scanf("%d%d%d", &xl, &xh, &n);

  std::vector<Event> events;
  REP (i, n) {
    auto &t = tarps[i];
    scanf("%d%d%d%d", &t.xl, &t.y1, &t.xh, &t.y2);
  }

  t_target = tarps + n++;
  t_src = tarps + n++;
  t_ceiling = tarps + n++;
  t_floor = tarps + n++;
  t_ceiling->set(xmin, xmax, yceiling, yceiling);
  t_floor->set(xmin, xmax, yfloor, yfloor);
  t_src->set(xl, xh, ysrc, ysrc);
  t_target->set(xl, xh, ytarget, ytarget);
  t_src->is_src = true;

  REP (i, n) {
    tarps[i].tid = i;
    tarps[i].setup();
  }

  REP (i, t_src->tid + 1) {
    auto &t = tarps[i];
    Event ev;
    ev.x = t.xl;
    ev.tid = i;
    events.pb(ev);
    ev.x = t.xh;
    events.pb(ev);
  }

  std::set<State> sl;
  sl.insert(State(t_ceiling->tid));
  sl.insert(State(t_floor->tid));

  std::sort(ALL(events));
  for (auto &ev : events) {
    auto &t = tarps[ev.tid];
    bool low = ev.x == t.xl;
    state_x = ev.x;

    // puts("");
    // OPA_DISP0(ev.x, ev.tid, t.y(state_x));
    // for (auto &k : sl) OPA_DISP0(tarps[k.tid].y(state_x));
    State s(ev.tid);
    if (!low) sl.erase(s);
    auto it = sl.lower_bound(s);
    assert(it != sl.begin());
    assert(it != sl.end());
    auto next = &tarps[it->tid];
    auto prev = &tarps[(--it)->tid];

    if (low) {
      prev->push(ev.x, &t);
      t.push(ev.x, next);
      sl.insert(s);

    } else {
      t.push(ev.x, nullptr);
      prev->push(ev.x, next);
    }
  }

  for (auto &lnk : t_src->downlinks) {
    if (lnk.target->downleft) {
      lnk.target->push(lnk.xl, 0);
      lnk.target->push(lnk.xh, 0);
    }
  }

  REP (i, n)
    tarps[i].finish_links();

  std::queue<Tarp *> q;
  q.push(t_floor);
  std::vector<Tarp *> order;
  while (!q.empty()) {
    auto t = q.front();
    q.pop();
    order.pb(t);
    // OPA_DISP0(t->tid, t->xl, t->y1);

    for (auto &uplink : t->uplinks) {
      auto x = uplink.target;
      if (++x->deg_seen == x->downlinks.size()) q.push(x);
    }
  }
  t_src->score.add(ScoreEntry(0, 0));
  // OPA_DISP0(t_src->score);
  assert(order.size() == n);
  std::reverse(ALL(order));
  assert(order[0] == t_ceiling);
  // assert(order[1] == t_src);
  assert(order.back() == t_floor);

  OPA_TRACE0;
  FOR (i, 2, order.size() - 1) { compute(order[i]); }

  int res = t_target->score.query_best();
  printf("%d\n", res);
  return 0;
}

