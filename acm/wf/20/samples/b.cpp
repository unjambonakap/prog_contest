...
struct SolverContext {
  int n, c1;
  std::vector<std::vector<pii> > el;
  std::unordered_map<int, int> rmp;
  vi mp;
  const s64 inf = 1e18;

  struct State {
    std::vector<s64> costs;
  };

  State go(int pos, int prev = -1) {

    std::vector<std::pair<int, State> > children;
    State res;
    res.costs.resize(mp.size(), inf);
    int mx = 0;
    s64 s = 0;
    int cprev = -1;
    for (auto e : el[pos]) {
      checkmax(mx, e.second);
      if (e.first == prev) {
        cprev = rmp[e.second];
        continue;
      }
      children.emplace_back(rmp[e.second], go(e.first, pos));
      s -= e.second;
    }
    s64 nx = children.size();
    int pi = rmp[mx];

    s64 c0 = 1ull * c1 * el[pos].size();
    for (auto &y : children) {
      s64 cx = inf;
      FOR (v, y.first, mp.size())
        checkmin(cx, (mp[v] - mp[y.first]) + y.second.costs[v]);
      c0 += cx;
    }

    FOR (v, pi, mp.size()) {
      s64 x = s + 1ull * nx * mp[v];
      assert(x >= 0);
      for (auto &y : children) x += y.second.costs[v];
      res.costs[v] = x;
    }
    if (prev == -1) {
      REP (v, mp.size())
        checkmin(res.costs[v], c0);
    } else {
      FOR (v, cprev, mp.size())
        checkmin(res.costs[v], c0);
    }
  ...
  s64 solve() {
    if (n==1) return 0;
    State res = go(0);
    return *std::min_element(ALL(res.costs));
  }
