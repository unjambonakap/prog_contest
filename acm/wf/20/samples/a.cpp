...
struct res_t {
  int p, i, j, s;
  int d;
  int dx() const { return std::abs(2 * i - r + 1) / 2 + std::abs(2 * j - c + 1) / 2; }
  bool operator<(const res_t &peer) const {
    if (d != peer.d) return d < peer.d;
    if (dx() != peer.dx()) return dx() < peer.dx();
    return p < peer.p;
  }
};
...
res_t get(int p) {
  ...
  int low = 0, high = r - 1;
  while (true) {
    s64 nlow = 1ull * p * r + low;
    s64 nhigh = 1ull * p * r + high;
    res.s++;
    nlow /= c;
    nhigh /= c;
    if (nlow == low && nhigh == high) return res;
    low = nlow;
    high = nhigh;
    if (low == high) {
      res.i = low;
      res.d = 1;
      res.j = (1ull*p*r+low)%c;
      return res;
    }
  }
}
