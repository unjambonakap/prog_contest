...
struct HashFunc {
  ...
  void compute() {
    ...
    REP (i, tb.size()) {
      pwx = pwx * pw % mod;
      pwtb.pb(pwx);
      cur = (cur * pw + tb[i] + 1) % mod;
      h.pb(cur);
    }
  }

  s64 get(int a, int n) const { return (mod + (h[a + n] - h[a] * pwtb[n]) % mod) % mod; }
};
struct SolverContext {
  ...
  s64 solve() {
    ...
    HashFunc hf, hfr;
    for (auto x : s) hf.tb.pb(mp[x]);
    hfr.tb = hf.tb;
    std::reverse(ALL(hfr.tb));
    hf.compute();
    hfr.compute();
    int l = 0, r = s.size() - 1;
    while (true) {
      bool fd = false;
      for (int n1 = 1; l + 2 * n1 - 1 <= r; ++n1) {
        if (hf.get(l + n1, n1) == hfr.get(s.size() - 1 - (l + n1 - 1), n1)) {
          l += n1;
          fd = true;
          break;
        }
        if (hf.get(r - n1 + 1, n1) == hfr.get(s.size() - 1 - (r - n1), n1)) {
          r -= n1;
          fd = true;
          break;
        }
