...
struct SolverContext {
  ...
  std::vector<pii> query(const std::vector<pii> &q) {
    vi qx(n, -1);
    for (auto x : q) qx[x.ST] = x.ND, qx[x.ND] = x.ST;
    printf("test");
    for (auto v : qx) printf(" %d", v + 1);
    puts("");
    fflush(stdout);
    g_reader.readline();
    std::vector<pii> same;
    for (auto x : q)
      if (g_reader.buf[x.ST] == '1' && g_reader.buf[x.ND] == '1') same.pb(x);
    return same;
  }
  std::unordered_set<pii> done;
  ...
  std::vector<pii> q1() {
    ...
    REP (i, n)
      rem.pb(i);
    std::shuffle(ALL(rem), rng);
    std::unordered_set<pii> seen;
    for (auto &d : done) seen.emplace(root(d.ST), root(d.ND));
    while (rem.size() > 1) {
      int a = rem.back();
      rem.pop_back();
      int ra = root(a);
      REP (j, rem.size()) {
        ...
        if (ra != rb && !seen.count(MP(ra, rb))) {
          seen.emplace(ra, rb);
          seen.emplace(rb, ra);
          q.emplace_back(a, b);
          std::swap(rem[j], rem.back());
          rem.pop_back();
          break;
        }
      }
  ...
  std::vector<vi> groups() {
    ...
    REP (i, n)
      tb[root(i)].pb(i);
    std::vector<vi> res;
    for (auto &e : tb) res.pb(e.second);
    std::sort(ALL(res), [](const vi &a, const vi &b) { return a.size() > b.size(); });
    return res;
  }
  ...
  void solve() {
    ...
    REP (i, 11) {
      auto r = query(q1());
      for (auto &x : r) join(x.ST, x.ND);
      auto grps = groups();
      if (grps[0].size() >= (n + 1) / 2) {
        std::vector<pii> q;
        FOR (j, 1, grps.size())
          q.emplace_back(grps[0][j - 1], grps[j][0]);
        auto r = query(q);
        for (auto &x : r) join(x.ST, x.ND);
