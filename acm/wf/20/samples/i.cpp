...
struct BitSet {
  int n;
  BitSet(int n) : n(n) { tb.resize((n + 63) / 64, 0); }
  void set(int a, int v) { if (a<n) tb[a / 64] |= v * 1ull << (a & 63); }
  void sor(int shift, int last) {
    int sa = shift % 64;
    int va = shift / 64;
    int sb = (64 - sa);
    int li = last / 64;
    REPV (i, 64)
      if (li * 64 + i <= last) set(li * 64 + i + shift, tb[li] >> i & 1);
    checkmin<int>(li, tb.size()-va);
    REPV (i, li) {
      if (sa != 0 && i + va + 1 < tb.size()) tb[i + va + 1] |= tb[i] >> sb;
      tb[i + va] |= tb[i] << sa;
    }
  }
  ...
  int query_max() const {
    REPV (i, tb.size())
      if (tb[i] != 0) return two_pow64(tb[i]) + 64 * i;
    return 0;
  }
  int getv(int a) const { return tb[a / 64] >> (a & 63)&1; }
...
struct SolverContext {
  ...
  s64 solve() {
    std::sort(ALL(tb), [&](const pii &a, const pii &b) {
      return a.first * c + a.second * v < b.first * c + b.second * v;
    });
    BitSet bs(maxx * (n+1));
    bs.set(0, 1);
    s64 sum = 0;
    for (auto e : tb) {
      sum += e.first;
      bs.sor(e.first, (e.second*v - 1) / c);
    }
    return sum + 1ull * (c - 1) * bs.query_max();
  }
