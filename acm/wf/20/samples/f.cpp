...
struct SolverContext {
  ...
  int solveit(int a) {
    ...
    REP (i, n) {
      if (i == a) continue;
      auto diff = pl[i] - pl[a];
      long double ang = atan2l(diff.y, diff.x);
      long double dx = pi;
      if (diff.dot(diff) > 1ull * t * t) {
        long double s = sqrtl((long double)diff.dot(diff));
        dx = asinl(t / s);
        intervals.emplace_back(ang + pi + 0 * 2*pi, ang + pi + dx + 0 *2*pi);
        intervals.emplace_back(ang + pi + 1 * 2*pi, ang + pi + dx + 1 *2*pi);
        intervals.emplace_back(ang + pi + -1 * 2*pi, ang + pi + dx + -1 *2*pi);
      }
      intervals.emplace_back(ang - dx-2*pi, ang-2*pi);
      intervals.emplace_back(ang - dx, ang);
      intervals.emplace_back(ang - dx + 2 * pi, ang + 2 * pi);
    }

    std::sort(ALL(intervals));
    OPA_DISP0(intervals);
    std::priority_queue<long double> pending;
    for (auto x : intervals) {
      assert(x.first < x.second);
      pending.emplace(-x.second);
      while (-pending.top() < x.first) {
        assert(pending.size() > 0);
        pending.pop();
      }
      checkmax<int>(best, pending.size());
    }
  ...
  int solve() {
    ...
    REP (i, n)
      pl.emplace_back(pts[i].ST, pts[i].ND);
    int best = 2;
    REP (i, n)
      checkmax(best, solveit(i));
    return best;
  }
  ...
  int solve2() {
    ...
    REP (i, n)
      REP (j, n)
        if (i != j) {
          Segment<s64> s1(pl[i], pl[j]);
          auto s2 = s1.orth_dir();
          int cnt = 0;
          auto d0 = s2.dot(pl[i]);
          s128 lim = (s128)s2.dot(s2) * t * t;
          REP (k, n) {
            auto d = s2.dot(pl[k]) - d0;
            if (d >= 0) {
              s128 u = (s128)d * d;
              cnt += u <= lim;
            }
