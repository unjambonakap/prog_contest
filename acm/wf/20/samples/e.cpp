...
struct Data {
  s64 ca = 0, cb = 0;
  void push(const Data &peer) {
    ca += peer.ca;
    cb += peer.cb;
  }
  void reset() { ca = cb = 0; }
  s64 value(int x) const { return ca + cb * x; }
};
...
struct Node {
  ...
  void add(int low, int high, Data nd) {
...
struct SolverContext {
  std::vector<s64> solve() {
    ...
    REP (i, nq) {
      ...
      --r;
      Data dd;
      if (op == 'R') dd.ca = 1, st.root->add(l, r, dd);
      if (op == 'D') dd.ca = -1, st.root->add(l, r, dd);
      if (op == 'H') {
        dd.cb = 1;
        dd.ca = -l+1;
        int m = (l + r) / 2;
    ...
    std::vector<s64> res(n);
    for (auto &x : st.nodes) {
      x.norm();
      if (x.l == nullptr) res[x.a] = x.data.value(x.a);
    }
