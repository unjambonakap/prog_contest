...
struct KMP {
  ...
  void compute(const vi &tb) {
    ...
    FOR (i, 1, n) {
      int x = i;
      while (x > 0 && tb[fail[x]] != tb[i]) x = fail[x];
      fail[i+1] = fail[x] + 1;
    }
  }
};
...
struct SolverContext {
  ...
  bool solve() {
    std::unordered_map<vi, int> mpx;
    std::map<int, int> grp_keyed[2];
    REP (f, 2) {
      std::map<int, vi> grps;
      for (auto x : p[f]) grps[x.ND].pb(x.ST);
      std::vector<int> keys;
      for (auto kv : grps) keys.pb(kv.first);
      keys.pb(keys[0] + 360 * 10000);
      REP (i, keys.size()-1)
        grps[keys[i]].pb(INT_MIN+(keys[i + 1] - keys[i]));

      for (auto &e : grps) {
        std::sort(ALL(e.second));
        grp_keyed[f][e.first] = mpx.emplace(e.second, mpx.size()).first->second;
      }
    }
    std::vector<int> search;
    int n = grp_keyed[0].size();
    if (grp_keyed[1].size() != n) return false;

    for (auto kv : grp_keyed[0]) search.pb(kv.second);
    search.pb(-1);
    for (auto kv : grp_keyed[1]) search.pb(kv.second);
    REP(i,n-1) search.pb(search[n+1+i]);
    KMP kmp;
    kmp.compute(search);
    for (auto v : kmp.fail)
      if (v == n) return true;
  ...
  void read() {
    ...
    REP (f, 2) {
      ...
      REP (i, n) {
        ...
        a *= 1e4;
        b *= 1e4;
        p[f].emplace_back(std::round(a), std::round(b));
      }
    }
  }
};
