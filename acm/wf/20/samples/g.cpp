...
struct Data {
  s64 v = 0;
  Data agg(const Data &y) const { return Data{ std::max(v, y.v) }; }
};
...
struct Node {
  ...
  void norm() {
    if (l == nullptr) return;
    data = l->data.agg(r->data);
  }
  Data query(int l, int r) const {
    if (b < l || r < a) return Data();
    if (l <= a && b <= r) return data;
    return this->l->query(l, r).agg(this->r->query(l, r));
  }

  void add(int p, Data nd) {
    if (p > b || p < a) return;
    if (l) {
      l->add(p, nd);
      r->add(p, nd);
...
struct TwoDimOpQ {
  ...
  void push(int ix, int iy, int vx, int vy, int add) {
    qx.root->add(ix, Data{ vy + add });
    qy.root->add(iy, Data{ vx + add });
    qxy.root->add(ix, Data{ vx + vy + add });
    checkmax<s64>(maxadd, add);
  }
  s64 query(int ix, int iy, int x, int y) {
    s64 res = 0;
    checkmax(res, qx.root->query(0, ix).v - y);
    checkmax(res, qy.root->query(0, iy).v - x);
    checkmax(res, qxy.root->query(ix, qx.n).v - x - y);
    checkmax(res, maxadd);
    return res;
  }
};
...
struct SolverContext {
  ...
  std::pair<s64, int> solve() {
    std::vector<s64> sols(n, 0);
    Remapper rmp;
    for (auto &e : entries) REP (f, 3)
        rmp.vals.insert(e.tb[f]);
    rmp.build();
    for (auto &e : entries) REP (f, 3)
        e.tb[f] = rmp.rmp[e.tb[f]];

    std::sort(ALL(entries), [](const Entry &a, const Entry &b) { return a.tb[0] < b.tb[0]; });
    TwoDimOpQ d1;
    TwoDimOpQ d2;
    d1.build(rmp.mp.size());
    d2.build(rmp.mp.size());
    FE (it, entries) {
      auto &e = *it;
      checkmax(sols[e.id], d1.query(e.tb[1], e.tb[2], rmp.mp[e.tb[1]], rmp.mp[e.tb[2]]));
      d1.push(e.tb[1], e.tb[2], rmp.mp[e.tb[1]], rmp.mp[e.tb[2]], 0);
    }

    FEV (it, entries) {
      auto &e = *it;
      checkmax(sols[e.id],
               d2.query(e.tb[1], e.tb[2], rmp.mp[e.tb[1]], rmp.mp[e.tb[2]]) - rmp.mp[e.tb[0]]);
      OPA_DISP0(e.id, sols[e.id], rmp.mp[e.tb[0]]);
      d2.push(e.tb[1], e.tb[2], rmp.mp[e.tb[1]], rmp.mp[e.tb[2]], rmp.mp[e.tb[0]]);
    }
    auto it = std::min_element(ALL(sols)) - sols.begin();
    return MP(sols[it], it);
  }
  ...
  void read() {
    ...
    REP (i, n) {
      auto &e = entries[i];
      e.id = i;
      scanf("%d%d%d", &e.tb[0], &e.tb[1], &e.tb[2]);
    }
  }
};
