...
struct SolverContext {
  ...
  bool inside(double x, double y) const {
    return x >= -eps && y >= -eps && x < dx + eps && y < dy + eps;
  }

  double solve() {
    std::vector<Segment<s64> > cut_lines;
    cut_lines.emplace_back(Point<s64>(0, 0), Point<s64>(dx, 0));
    cut_lines.emplace_back(Point<s64>(dx, 0), Point<s64>(dx, dy));
    cut_lines.emplace_back(Point<s64>(dx, dy), Point<s64>(0, dy));
    cut_lines.emplace_back(Point<s64>(0, dy), Point<s64>(0, 0));
    HullHelper hh2;
    REP (i, n)
      hh2.pts.pb(pts[i].as<double>());
    auto hull1 = hh2.get_hull();
    std::map<int, int> posmap;
    REP (i, n)
      posmap[order[i]] = i;

    if (1) {
      REP (i, n - 1) { cut_lines.emplace_back(pts[order[i + 1]], pts[order[i]]); }
      REP (i, hull1.size() - 1) {
        if (posmap[hull1[i]] < posmap[hull1[i + 1]]) {
          cut_lines.emplace_back(pts[hull1[i + 1]], pts[hull1[i]]);
        }
      }
    } else
      REP (i, n)
        REP (j, i)
          cut_lines.emplace_back(pts[order[i]], pts[order[j]]);
    std::set<Point<double> > cpoints, cnds;

    REP (i, cut_lines.size())
      REP (j, i) {
        if (cut_lines[i].parallel(cut_lines[j])) continue;
        auto px = cut_lines[i].as<double>().inter_pointd(cut_lines[j].as<double>());
        bool in = true;
        REP (i, hull1.size() - 1)
          if (Segment<double>(hh2.pts[hull1[i]], hh2.pts[hull1[i + 1]]).signed_dist(px) < eps) {
            in = false;
            break;
          }
        if (in) continue;
    ...
    for (auto i : hull1) cnds.insert(hh2.pts[i]);

    for (auto px : cnds) {
      bool ok = true;
      for (auto e : cut_lines)
        if (e.as<double>().signed_dist(px) < -eps) {
          ok = false;
          break;
        }
      if (ok) cpoints.insert(px);
    }

    OPA_DISP0(cpoints);
    if (cpoints.size() <= 2) return 0;
    HullHelper hh;
    for (auto pt : cpoints) hh.pts.pb(pt);
    vi ids = hh.get_hull();

    double area = 0;
    REP (i, ids.size() - 1) {
      auto a = hh.pts[ids[i]];
      auto b = hh.pts[ids[i + 1]];
  ...
  void read() {
    ...
    pts.resize(n);
    REP (i, n)
      scanf("%Ld%Ld", &pts[i].x, &pts[i].y);
    order.resize(n);
    REP (i, n)
      scanf("%d", &order[i]), --order[i];
  }
};
