...
struct SolverContext {
  ...
  std::array<int, 5> go(int a, int prev) {
    std::array<int, 5> res;
    res.fill(0);
    for (auto &x : edges[a]) {
      if (x.first == prev) continue;
      std::array<int, 5> nres;
      auto cur =  go(x.first, a);
      nres.fill(0);
      REP(i,5) REP(j,5-i)
        checkmax(nres[i+j], res[i] + cur[j] + (j&1?x.second:0));
      res = nres;
    }
  ...
  int solve() { 
    int tot=0;
    REP(i,n) for (auto e : edges[i]) tot += e.second;
    int res =  go(0, -1)[4]; 
    return tot-res;
  }
