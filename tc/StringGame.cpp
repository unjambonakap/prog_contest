...
class StringGame{ 
    ...
    vector <int> getWinningStrings(vector <string> S){ 
        ...
        REP(i,n) REP(j,S[i].size()) ++freq[i][S[i][j]-'a'];
        
        REP(i,n) REP(k,n) REP(j,26){
            if (freq[i][j]==freq[k][j]) wx[i][j]|=1ll<<k;
            else if (freq[i][j]<freq[k][j]) lx[i][j]|=1ll<<k;
        }

        REP(i,n){
            ll x=(1ll<<n)-1-(1ll<<i);
            REP(j,26) REP(k,26) if (!(lx[i][k]&x)) x&=wx[i][k];
            if (!x) res.pb(i);
