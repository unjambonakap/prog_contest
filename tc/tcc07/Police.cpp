...
void dfs(int p){
	...
	REP(i, n) if (graph[p][i]){
		if (lnk[i]==-1)dfs(i), checkmin(lnk[p], lnk[i]);
		else if (stk[i]) checkmin(lnk[p], lnk[i]);
	}
...
class Police {
	...
	int minimalCost(vector <int> cost, vector <string> roads) {
		CLR(scc);
		n=roads.size();
		REP(i, n) REP(j, n) graph[i][j]=roads[i][j]=='Y';
		...
		FE(it, scc){
			int nc=INT_MAX;
			FE(ita, *it) checkmin(nc, cost[*ita]);
