...
ll proc(int n, int k, int l){
	...
	if (f==n){
		return r=proc(n-1, k-1, 0);
	}
	r=0;
	REP(i, l) r+=proc(n-1, k-1, i);
	FOR(i, l, n-1) r+=proc(n-1, k, i);
...
class KSlopePermutation {
	public:
	long long getCount(int n, int k, int _f) {
		memset(dp, -1, sizeof(dp));
		ll res=0;
		f=n-_f;
		if (n==1) return (k==0 && _f==0);
		if (f==n) return proc(n-1, k, 0);
		REP(i, n)
			res+=proc(n-1, k, i);
		return res;
...
// BEGIN CUT HERE
	...
	void test_case_0() { int Arg0 = 1; int Arg1 = 0; int Arg2 = 0; long long Arg3 = 4LL; verify_case(0, Arg3, getCount(Arg0, Arg1, Arg2)); }
