...
class Over9000Rocks {
	...
	int countPossibilities(vector <int> lb, vector <int> ub) {
        ...
        REP(i,n){
            vector<pii> ncan=can;
            REP(j,can.size()) ncan.pb(MP(can[j].ST+lb[i],can[j].ND+ub[i]));
            int last=-1;
            sort(ALL(ncan));
            can.clear();
            REP(j,ncan.size()){
                if (last<ncan[j].ST) can.pb(ncan[j]);
                else checkmax(can.back().ND,ncan[j].ND);
                checkmax(last,ncan[j].ND);
            }
        }
        int res=0;
    
        REP(i,can.size()){
            if (can[i].ND<9001) continue;
            res+=can[i].ND-max(can[i].ST,9001)+1;
        }
        return res;
	}
...
// BEGIN CUT HERE
	...
	void test_case_0() { int Arr0[] = 
        {9000,1,1}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = 
        {9001,1,1}; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); int Arg2 = 1; verify_case(0, Arg2, countPossibilities(Arg0, Arg1)); }
