...
int checkit(int a, int b){
    ll best=0;
    int u=A.size()?A.back():0;
    int v=0;
    int nb=B.size();
    REP(i,nb) v+=B[i];
    checkmax(best,1ll*a*u);
    if (nb){
        ll nbest=1ll*v*(a/nb);
        a%=nb;
        int tmp=0;
...
class BlurredDartboard {
	public:
	int minThrows(vector <int> pl, int P) {
        n=pl.size();
        A.clear(); B.clear();
        REP(i,n) if (pl[i]) A.pb(pl[i]);
        FOR(i,1,n+1){
            int fd=0;
            REP(j,A.size()) if (A[j]==i){fd=1; break;}
            if (!fd) B.pb(i);
        }
        sort(ALL(A));
        sort(ALL(B));
