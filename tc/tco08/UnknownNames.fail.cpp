...
class UnknownNames {
	...
	vector <string> sortNames(vector <string> names) {
		...
		REP(i, n) {
			...
			REP(k, l) if (names[i][k]!='?') dl[i][k]=max(dl[i][k], names[i][k]);
		...
		FE(it, temp){
			...
			if (dl[it->ST]<dl[it->ND]){graph[it->ST][it->ND]=1; continue;}
			if (dl[it->ND]<dl[it->ST]){graph[it->ND][it->ST]=1; continue;}

			if (graph[it->ST][it->ND]==0 && graph[it->ND][it->ST]==0){
				printf("<<<<<<<<<<<<\n");
				REP(k, l) 
					if (names[it->ST][k]=='?'&&names[it->ND][k]!='?'){graph[it->ST][it->ND]=1;break;}
					else if (names[it->ST][k]!='?'&&names[it->ND][k]=='?'){graph[it->ND][it->ST]=1;break;}
				if (!graph[it->ST][it->ND]&&!graph[it->ND][it->ST]){
					graph[it->ST][it->ND]=1;
		...
		while(!q.empty()){
			int p=q.front(); q.pop();
			if (num[p]!=-1)return vs();
			num[remap[na]=p]=na++;
			REP(i, n) if (graph[p][i]) if (--deg[i]==0) q.push(i);
		}
		REP(i, n) if (num[i]==-1)
			return vs();
		...
		REP(i, n){
			...
			REP(j, l) if (res[i][j]=='?') res[i][j]=(i==0)?'A':res[i-1][j];
			cout<<res[i]<<endl;
		}
		exit(0);
		return res;
