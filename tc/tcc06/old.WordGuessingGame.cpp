...
int solve(int a, int b){
    ...
    vs ans;
    REP(i, maxc){
        int s=solve(a+1, b^f[a][i]);
        FE(it, sol[s]) ans.pb(string(1, i+'A')+*it);
    }
    r=(ans.size()==0?0:ns++);
    sol[r]=ans;
    return r;
...
class WordGuessingGame {
    public:
        vector <string> possibleWords(vector <string> even, vector <string> odd) {
            ...
            REP(i, even.size()) REP(j, n) tb[i+odd.size()][j]=even[i][j]-'A';
            
            memset(f, 0, sizeof(f));
            REP(i, n) REP(j, maxc) REP(k, m) if (tb[k][i]==j) f[i][j]|=two(k);

            ns=2;
            REP(i, maxs) sol[i].clear();
            sol[1].pb(string());


            memset(mem, -1, sizeof(mem));
            return sol[solve(0, b)];
        }
