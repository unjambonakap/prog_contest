...
int vx[]={0,1,0,-1};
int vy[]={1,0,-1,0};
void proc(int ida, int idb, int xa, int ya, int da, int xb, int yb, int db, priority_queue<pair<int,pii> > &q){
    REP(t,4444){
        int nxa=xa+t*vx[da], nya=ya+t*vy[da];
        int nxb=xb+t*vx[db], nyb=yb+t*vy[db];
        if (nxa==nxb && nya==nyb){
            q.push(MP(-t,MP(ida,idb)));
            return;
        }
    }
}
...
class AntsMeet {
	...
	int countAnts(vector <int> x, vector <int> y, string direction) {
        ...
        REP(i,n) REP(j,n) if (direction[i]!=direction[j]){
            int da=-1, db=-1;
            REP(k,4) if (direction[i]==str[k]) da=k;
            REP(k,4) if (direction[j]==str[k]) db=k;
            proc(i,j,x[i],y[i],da,x[j],y[j],db,q);
        }
        memset(has,0x7f,sizeof(has));
        while(!q.empty()){
            int t=-q.top().ST;
            int a=q.top().ND.ST, b=q.top().ND.ND; q.pop();
            if (has[a]<t || has[b]<t) continue;
            has[a]=has[b]=t;
        }

        int res=0;
        REP(i,n) if (has[i]>1e8) ++res;
        return res;
...
// BEGIN CUT HERE
	...
	void test_case_0() { int Arr0[] = {-1000,1000}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = {1000,-1000}; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); string Arg2 = "EN"; int Arg3 = 2; verify_case(0, Arg3, countAnts(Arg0, Arg1, Arg2)); }
