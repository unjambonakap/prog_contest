...
class FirstToLast {
	...
	int getSmallest(int p, int q) {
        ...
        int res[maxp];
        REP(e, 9) REP(k, 10) if (k){
            ll v=(pow(10., e+1)*k-k)*q;
            if (v%a==0 && v/a<pow(10., e)*(k+1)) return v/a;
            
        }
        int e=9;
        REP(k, 2) if (k){
            ll v=ll(pow(10., e+1)*k-k)*q;
            if (v%a==0 && v/a<pow(10., e)*(k+1)) return v/a;
        }
        return -1;
...
// BEGIN CUT HERE
	...
	void test_case_0() { int Arg0 = 90; int Arg1 = 8; int Arg2 = 1; verify_case(0, Arg2, getSmallest(Arg0, Arg1)); }
