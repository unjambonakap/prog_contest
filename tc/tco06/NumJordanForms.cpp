...
class NumJordanForms {
	public:
	int howMany(vector <int> va, vector <int> vb) {
        n=va.size();
        ...
        REP(i, maxv) if (i){
            ...
            REP(j, maxv-i) dpa[j+i]=(dpa[j]+(i==1?0:dp[i-1][j]))%mod;
            REP(j, maxv) dp[i][j]=((i==1?1:dp[i-1][j])+dpa[j])%mod;
            REP(j, n) if (vb[j]==i) ans=ans*dp[i][va[j]-vb[j]]%mod;
