...
// END CUT HERE
#line 5 "SeparateConnections.cpp"
...
int solve(int p, int m){
    ...
    int &r=dp[m][p];
    if (r!=-1) return r;
    if (m&two(p)) return r=solve(p+1, m);

    r=solve(p+1, m);
    FOR(i, p+1, n-1) if (!(m&two(i)) && mp[i][p]) checkmax(r, solve(p+1, m|two(i))+1);
    return r;
...
class SeparateConnections {
	...
	int howMany(vector <string> mat) {
        n=mat.size();
        REP(i, n) REP(j, n) mp[i][j]=mat[i][j]=='Y';
