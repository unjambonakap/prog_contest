...
class AmbiguousWatch {
	public:
	int howMany(string st, string ft) {
		st[2]=ft[2]=' ';
		int ha, ma, ht, mt;
		istringstream(st)>>ha>>ma;
		istringstream(ft)>>ht>>mt;

		int ta, tb; ta=ha*60+ma; tb=ht*60+mt;
		if (tb<ta) tb+=12*60;
		int cnt=0;
		FOR(i, ta, tb-1){
			if (ma==60)ma=0, ++ha;
			...
			int va, vb; va=3600*hb-60*12*ma+60*mb; vb=3600*ha-60*12*mb+60*ma;
			double sa=double(12*va+vb)/(12*12-1), sb=12*sa-va;
			//if ((ha!=hb || ma!=mb) && sa>=0 && sa<60 && sb>=0 && sb<60) printf("%d %d // %d %d =>>>>>>< %lf %lf\n", ha, ma, hb, mb, sa, sb);
			if ((ha!=hb || ma!=mb) && sa>=0 && sa<60 && sb>=0 && sb<60)++cnt;
			++ma;
