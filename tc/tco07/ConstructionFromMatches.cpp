...
// END CUT HERE
#line 5 "ConstructionFromMatches.cpp"
...
const int maxn=55, maxv=13, inf=1000000;
...
int proc(int p, int a, int b){
	if (a>=m || b>=m)return -2;
	if (p==n)return cost[a]+cost[b];
	int &r=dp[p][a][b];
	if (r!=-1)return r;
	r=-2;
	if (tb[p][0]<a || tb[p][1]<b)return r;

	REP(va, m) REP(vb, m) REP(vm, m){
		int sa=tb[p][0]-va-vm-a, sb=tb[p][1]-vb-vm-b;
		if (sa<0 || sb<0)continue;
		int t=proc(p+1, sa, sb);
		if (t==-2)continue;
		t+=cost[va]+cost[vb]+cost[vm]+cost[a]+cost[b];
		if (r==-2 || t<r)r=t;
	}
	return r;
...
class ConstructionFromMatches {
	public:
	int minimumCost(vector <int> _cost, vector <int> top, vector <int> bottom) {
		cost=_cost;
		n=top.size(); m=cost.size();
		REP(i, n)tb[i][0]=top[i]-4, tb[i][1]=bottom[i]-4;
		int v=-1, t;
		memset(dp, -1, sizeof(dp));
		REP(i, m) REP(j, m) if ((t=proc(0, i, j))!=-2 && (v==-1 || t<v))v=t;
		return v==-2?-1:v;
	}
