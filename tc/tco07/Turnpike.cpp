...
struct cmpa{ bool operator()(pii a, pii b){return double(a.ST)/(1+a.ND)<double(b.ST)/(1+b.ND);} };
...
class Turnpike {
	...
	int unserviced(int pikeLength, int n, vector <int> plazas) {
		...
		sort(ALL(plazas));
		multiset<pii, cmpa> e;
		if (m) e.insert(MP(plazas[0], 0)), e.insert(MP(pikeLength-plazas.back(), 0));
		else e.insert(MP(pikeLength, 0));
		REP(i, m-1) e.insert(MP(plazas[i+1]-plazas[i], 0));
		REP(i, n){
			pii a=*e.rbegin();
			e.erase(--e.end());
			e.insert(MP(a.ST, a.ND+1));
		}
		pii a=*e.rbegin();
		return ceil(double(a.ST)/(a.ND+1));
	}
