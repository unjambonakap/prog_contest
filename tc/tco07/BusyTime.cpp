...
class BusyTime {
	...
	double busy(int k, vector <int> need) {
		sort(ALL(need));
		if (need.size()<k)return 0.;
		int s=0;
		FE(it, need) s+=*it;
		REPV(i, need.size()){
			if (need[i]>s/k) --k, s-=need[i];
			else return double(s)/k;
		}
		return s;
...
// BEGIN CUT HERE
	...
	void test_case_0() { int Arg0 = 3; int Arr1[] = {100,100,100}; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); double Arg2 = 0.0; verify_case(0, Arg2, busy(Arg0, Arg1)); }
