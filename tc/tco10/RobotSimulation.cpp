...
class RobotSimulation {
	...
	int cellsVisited(string program, int times) {
		...
		seen.clear();
		REP(i, n){
			if (program[i]=='L') np[i]=3;
			else if (program[i]=='R') np[i]=1;
			else if (program[i]=='U') np[i]=0;
			else np[i]=2;
		}
		int x, y;
		x=y=0;
		int nstep=1000;
		REP(j, min(times, nstep)){
			seen.insert(MP(x, y));
			REP(i, n){
				x=x+vx[np[i]];
				y=y+vy[np[i]];
		...
		if (nstep<times){
			...
			return seen.size()+(seen.size()-od)*(times-nstep-1);
		}
		return seen.size();
