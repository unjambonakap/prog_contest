...
struct state{
	...
	ll c;
	int fueled;
	state(int a, int b, ll c, int fueled):a(a), b(b), c(c), fueled(fueled){}
	bool operator<(const state &s)const{
		if (c!=s.c)return c>s.c;
		if (a!=s.a) return a<s.a;
		if (b!=s.b) return b<s.b;
		return fueled<s.fueled;
	}
};
...
struct nstate{
	int a, b; 
	int fueled;
	nstate(int a, int b, int fueled):a(a), b(b), fueled(fueled){}
...
class LongJourney {
	...
	long long minimumCost(vector <int> fp, int ft, vector <string> roads) {
		...
		REP(i, n) dist[i][i]=0;
		REP(i, n) REP(j, n) REP(k, n) if (dist[i][k]!=-1 && dist[k][j]!=-1 && (dist[i][j]==-1 || dist[i][j]>dist[i][k]+dist[k][j]))  dist[i][j]=dist[i][k]+dist[k][j];
		REP(i, n) REP(j, n) if (dist[i][j]>ft) dist[i][j]=-1;

//		REP(i, n){ REP(j, n) printf("%d ", dist[i][j]); printf("\n"); }
		...
		while(!q.empty()){
			state p=q.top(); q.pop();
			if (seen.count(nstate(p.a, p.b, p.fueled)))continue;
			if (p.a==1) return p.c;
			seen.insert(nstate(p.a, p.b, p.fueled));

			REP(i, n) if (dist[p.a][i]!=-1){
				REP(f, 2) if (!seen.count(nstate(i, p.a, f))){
					ll nc;
					int ask=(f?ft:dist[p.a][i]);
					int rf=(p.fueled?ft-dist[p.b][p.a]:0);
					nc=ll(fp[p.a])*max(0, ask-rf)+p.c;
					q.push(state(i, p.a, nc, f));
				}
			}
		}
		return -1;
	...
	//void test_case_0() { int Arr0[] = {5,6,1,2}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 100; string Arr2[] = {"0,2,2 "
