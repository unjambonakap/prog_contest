...
class ColorfulChocolates {
	...
	int maximumSpread(string s, int ms) {
        ...
        REP(i,n) REP(c,26){
            vi a,b;
            REP(k,i+1) if (s[k]=='A'+c) a.pb(i-k);
            FOR(k,i+1,n) if (s[k]=='A'+c) b.pb(k-i);
            reverse(ALL(a));
            int u=0;
            REP(j,a.size()){
                u+=a[j]-j;
                int v=u;
                if (v<=ms) res=max(res,j+1);
                REP(k,b.size()){v+=b[k]-k-1; if (v<=ms) res=max(res,j+k+2);}
            }
        }
        return res;
...
// BEGIN CUT HERE
	...
	void test_case_0() { string Arg0 = "ABC"; int Arg1 = 1; int Arg2 = 2; verify_case(0, Arg2, maximumSpread(Arg0, Arg1)); }
