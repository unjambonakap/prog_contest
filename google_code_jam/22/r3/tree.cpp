...
#define two(x) (1LL << (x))
#define two_pow32(x) (32-__builtin_clz(x))
#define two_pow64(x) (64-__builtin_clzll(x))
...
struct GlobalSim {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      g_reader.readline() >> tn;
  }

  void checkit() {
    // if (TEST_RAND) assert(scores >= p);
  }

  void push(s64 score) { scores += score; }
};
GlobalSim g_sim;
...
struct TreeRepr {
  int root = 0;
  mutable std::unordered_map<int, Node> nodes;
  int n() const { return std::max<int>(1, nodes.size()); }
  std::unordered_set<int> blacklist;

  std::vector<int> get_edges(int x) const {
    vi res;
    for (auto y : nodes[x].edges)
      if (!blacklist.count(y)) res.pb(y);
    return res;
  }
  void apply(const Play &p) { blacklist.insert(ALL(p.bl)); }

  std::vector<Play> list_plays() {
    std::vector<Play> res;
    for (auto x : nodes) {
      if (blacklist.count(x.first)) continue;
      vi pe = get_edges(x.first);
      REP (j, 1 << pe.size()) {
        Play p;
        p.bl.pb(x.first);
        REP (k, pe.size())
          if (j >> k & 1) p.bl.pb(pe[k]);
        res.pb(p);
      }
    }
    return res;
...
struct Precomp {
  ...
  std::deque<TreeDesc> trees;
  std::unordered_map<vi, int> mp;
  std::deque<int> grundy;

  void from_repr(const TreeRepr &r, int x, int par, vi &sl) {
    vi desc;
    for (auto y : r.nodes[x].edges) {
      if (y == par) continue;
      from_repr(r, y, x, sl);
      if (!r.blacklist.count(x) && !r.blacklist.count(y)) {
        desc.pb(sl.back());
        sl.pop_back();
      }
    }
  ...
  TreeRepr to_repr(int tid) {
    TreeRepr repr;
    repr.root = 0;
    repr.nodes[repr.root];
    int offset = 1;

    for (auto child : trees[tid].children) {
      auto xx = to_repr(child);

      for (auto kv : xx.nodes) {
        for (auto y : kv.second.edges) repr.nodes[offset + kv.first].edges.insert(y + offset);
      }
      repr.nodes[xx.root + offset].edges.insert(repr.root);
      repr.nodes[repr.root].edges.insert(xx.root + offset);
      offset += xx.n();
    }
    return repr;
  }

  int push_tree(const vi &tb) {
    int idx = trees.size();
    TreeDesc x;
    x.children = tb;
    x.vcount = 1;
    for (auto y : tb) x.vcount += trees[y].vcount;
  ...
  bool rec(int cur_count, int target, vi &desc, int v) {
    if (cur_count == target) {
      push_tree(desc);
      return true;
    }

    if (desc.size() > maxdeg) return true;
    desc.pb(0);
    for (; target >= cur_count + trees[v].vcount; ++v) {
      if (trees[v].vcount > maxchildcount) break;
      desc.back() = v;
      if (!rec(cur_count + trees[v].vcount, target, desc, v)) return false;
    }
    desc.pop_back();
    return true;
  }

  int get_score(const TreeRepr &tree, const Play &play) {
    TreeRepr ntr = tree;
    vi nstates;
    ntr.apply(play);
    from_repr(ntr, ntr.root, -1, nstates);
    int x = 0;
    for (auto v : nstates) {
      // assert(v < a);
      x ^= get_grundy(v);
  ...
  int get_grundy(int a) {
    if (grundy.size() <= a) grundy.resize(a + 1, -1);
    int &r = grundy[a];
    if (r != -1) return r;
    TreeRepr tr = to_repr(a);
    auto plays = tr.list_plays();
    std::unordered_set<int> mex;
    for (auto p : plays) {
      mex.insert(get_score(tr, p));
    }
    int vx = 0;
    for (; mex.count(vx); ++vx)
      ;
  ...
  int rec_grundy(int cur_count, int target, vi &desc, int v) {
    if (cur_count == target) {
      int idx = push_tree(desc);
      if (get_grundy(idx) == 0) return idx;
      return -1;
    }

    if (desc.size() >= 3) return -1;
    int res = -1;
    desc.pb(0);
    for (; v < trees.size() && target >= cur_count + trees[v].vcount; ++v) {
      int ev = v;
      int nc = cur_count + trees[v].vcount;
      if (trees[v].vcount >= target / 5 - 2) {
        while (nc < target) ev = push_tree({ ev }), ++nc;
      }
      desc.back() = ev;
      res = rec_grundy(nc, target, desc, v);
      if (res != -1) return res;
    }
    desc.pop_back();
    return res;
  }
  std::unordered_map<int, int> good_trees;
  ...
  void setup() {
    ...
    grundy.resize(trees.size(), -1);
    std::set<int> ns;
    REP (i, trees.size()) {
      if (get_grundy(i) == 0) ns.insert(trees[i].vcount);
    }

    FOR (tg, TG, TG2 + 1) {
      vi desc;
      int res = rec_grundy(1, tg, desc, 0);
      OPA_DISP0(res);
      if (res == -1)
        REP (i, trees.size())
          if (trees[i].vcount == tg) {
            res = i;
            break;
          }
      good_trees[tg] = res;
...
struct SolverContext {
  ...
  void solve() {
    int tid = g_precomp.good_trees[n];
    auto tree0 = g_precomp.to_repr(tid);
    for (auto &kv : tree0.nodes)
      for (auto y : kv.second.edges) {
        if (kv.first < y) printf("%d %d\n", kv.first + 1, y + 1);
      }
    ...
    REP (mm, m) {
      ...
      while (true) {
        int nx;
        g_reader.readline() >> nx;
        g_reader.readline();
        Play p;
        p.bl.resize(nx);

        REP (i, nx) g_reader.iss >> p.bl[i], --p.bl[i];
        for (auto x : p.bl) assert(tree.blacklist.count(x) == 0);
        auto s0 = g_precomp.get_score(tree, p);
        assert(s0 != 0);
        tree.apply(p);
        bool fd = false;
        for (auto p : tree.list_plays()) {

          if (g_precomp.get_score(tree, p) == 0) {
            for (auto x : p.bl) assert(tree.blacklist.count(x) == 0);
            printf("%lu\n", p.bl.size());
            for (auto x : p.bl) printf("%d ", x + 1);
            puts("");
            fflush(stdout);
        ...
        assert(fd);
        if (tree.blacklist.size() == n) break;
      }
    }
  ...
  void test() {
    ...
    REP (mm, m) {
      ...
      while (true) {
        ...
        bool fd = false;
        puts("NOW play");
        fprintf(stderr, "Run >> %d %d\n", s0, g_precomp.get_score(tree, p0));
        OPA_DISP0(tree.blacklist);
        for (auto p : tree.list_plays()) {
          OPA_DISP0("DRY >> ", p.bl);
