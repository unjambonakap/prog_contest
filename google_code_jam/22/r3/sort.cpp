...
struct Sim {
  void gen_rand() {}
  vi send(const vi &cl) {
    for (auto x : cl) printf("%d ", x + 1);
    puts("");
    fflush(stdout);

    vi state;
    int val;
    g_reader.readline() >> val;
    if (val == 1) return state;
    state.resize(g_sim.n);
    g_reader.readline();
    REP (i, g_sim.n) g_reader.iss >> state[i], --state[i];
    return state;
  }
    ...
    while (true) {
      ...
      REP (ic, cycles.size()) {
        ...
        if (0) {
          ...
          for (int j = 0; j < cl.size(); j += 4, ++cx)

            REP (k, 4)
              if (j + k < cl.size()) sx[cl[j + k]] = cx;
        }
      }
      state = sim.send(sx);
