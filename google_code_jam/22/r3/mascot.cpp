...
struct SolverContext {
  ...
  const int c = 13;
  std::vector<vi> el0;
  std::vector<std::unordered_set<int> > el;
  std::vector<vi> rel0;
  ...
  void read() {
    if (TEST_RAND) {
      ...
      REP (f, 2) {
        g_reader.readline();
        REP (i, n) g_reader.iss >> el0[i][f], --el0[i][f], rel0[el0[i][f]].pb(i);
      }
    }
  }
  ...
  res_t solve() {
    ...
    for (auto v : order) {
      ...
      for (auto y : el[v]) used.insert(res[y]);
      REP (j, c)
        if (!used.count(j)) {
          res[v] = j;
          break;
  ...
  void disp(const res_t &res) {
    std::string mp;
    const char *mpx = "ACDEHIJKMORST";
    for (auto x : res) mp += mpx[x];
    if (mp == "") mp = "IMPOSSIBLE";
    printf("%s\n", mp.c_str());
  }
};
