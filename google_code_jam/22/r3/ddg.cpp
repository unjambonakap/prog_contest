...
struct Data {
  ...
  int maxv = 0;
  int max_count = 1;
  void init_empty() { maxv = -oo; }

  void agg(const Data &a) {
    if (maxv > a.maxv + a.sum) return;
    if (maxv < a.maxv + a.sum) {
  ...
  void push(const Data &peer) { sum += peer.sum; }
  void reset() { sum = 0; }
};
...
struct Node {
  ...
  void query(int low, int high, Data &cur) {
    ...
    if (low <= a && b <= high)
      ...
      l->query(low, high, cur);
      r->query(low, high, cur);
    }
  }
};
...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      REP (i, c) {
        ranges[i].ST = rng() % n;
        ranges[i].ND = rng() % n;
        if (ranges[i].ST > ranges[i].ND) std::swap(ranges[i].ST, ranges[i].ND);
      }

      REP (i, n) state[i] = rng() % c;

    } else {
      g_reader.readline() >> n >> c;
      ranges.resize(c);
      state.resize(n);

      REP (i, c) g_reader.readline() >> ranges[i].ST >> ranges[i].ND;

      g_reader.readline();
      REP (i, n) g_reader.iss >> state[i], --state[i];
    }
    REP (i, n - 1) state.pb(state[i]);
  }

  std::vector<pii> get_intervals(int p, int cx = -1) {
    std::vector<pii> res;
    if (cx == -1) cx = state[p];
    const auto &cl = tb[cx];
    int i = p == -1 ? -1 : rpos[p];
    if (i == cl.size() - 1)
      res.emplace_back(p + 1, N);
    else {
      res.emplace_back(p + 1, cl[i + 1]);
      auto &r = ranges[cx];

      int li = i + std::max(r.ST, 1);
      if (r.ND != 0) {
        int pl = li < cl.size() ? cl[li] : N;
        int pr = i + r.ND + 1 < cl.size() ? cl[i + r.ND + 1] : N;
  ...
  void push(const std::vector<pii> &tb, int add) {
    Data d1;
    d1.sum = add;
    for (auto e : tb) 
      st.root->add(e.ST, e.ND - 1, d1);
  }
  ...
  res_t solve() {
    ...
    REP (i, state.size()) {
      rpos[i] = tb[state[i]].size();
      tb[state[i]].pb(i);
    }
    st.build(N);
    std::vector<std::vector<pii> > used(c);
    REP (cx, c) {
      auto ir = get_intervals(-1, cx);
      push(ir, 1);
      used[cx] = ir;
    }
    ...
    res_t res = 0;
    REP (i, n) {
      auto dx = st.query(i + 1, i + n - 2);
      if (dx.maxv == c) res += dx.max_count;
      push(used[state[i]], -1);
      auto ir = get_intervals(i);
      push(ir, 1);
      used[state[i]] = ir;
    }
  ...
  res_t solve_dumb() {
    ...
    REP (i, n)
      FOR (j, 1, n - 1) {
        vi cnt(c, 0);
        REP(k, j+1) cnt[state[i+k]] ++;
        res++;
        REP(k, c) if (cnt[k] && (cnt[k] < ranges[k].ST || cnt[k] > ranges[k].ND)){--res; break;}
      }
    return res;
  }
