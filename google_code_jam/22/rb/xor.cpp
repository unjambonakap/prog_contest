...
u8 rot(u8 a, int v) {
  v %= 8;
  return ((a >> v) | (a << (8 - v))) & 0xff;
}

std::vector<u8> rots(u8 a) {
  std::vector<u8> res;
  REP (i, 8)
    res.pb(rot(a, i));
  return res;
...
typedef u8 sol;
std::vector<u8> act(u8 a, u8 g) {
  std::vector<u8> res;
  for (auto v : rots(g)) res.pb(a ^ v);
  return res;
}
...
struct Precomp {
  ...
  void setup() {
    std::map<u8, int> dist;
    std::vector<int> q;
    auto push = [&](int a, int d) {
      if (dist.emplace(a, d).second) q.pb(a);
    };
    ...
    while (!q.empty()) {
      ...
      REP (i, 256) {
        ...
        REP (j, 256) {
          auto x = act(i, j);
          bool ok = true;
          for (auto k : x) ok &= dist.count(k);
          if (ok) {
            if (lst.size() == 0) chain.pb(j);
            lst.pb(i);
            break;
          }
        }
      }
      for (auto e : lst) push(e, step);
    }

    std::map<int, vi> plans;
    REP (i, chain.size()) {
      vi plan;
      plan.pb(chain[i]);
      REP (j, i)
        plan.insert(plan.end(), ALL(plans[j]));
      plans[i] = plan;
    }
    REP (i, chain.size())
      plan.insert(plan.end(), ALL(plans[i]));
  }
};
Precomp g_precomp;
...
struct SolverContext {
  ...
  void solve() {
    ...
    for (auto x : g_precomp.plan) {
      int cur;
      REP (i, 8)
        printf("%d", x >> i & 1);
      puts("");
      fflush(stdout);
      g_reader.readline() >> cur;
      assert(cur != -1);
      if (cur == 0) return;
    }
    assert(0);
