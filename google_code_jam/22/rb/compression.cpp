...
struct SolverContext {
  ...
  s64 solve() {
    pii val = MP(0, 0);
    std::pair<s64, s64> state = MP(0, 0);

    REP (i, n) {
      std::sort(ALL(tb[i]));
      pii nval = MP(tb[i][0], tb[i].back());
      int nx = nval.ND - nval.ST;
      auto s0 = std::min(state.ST + std::abs(val.ST - nval.ND) + nx,
                         state.ND + std::abs(val.ND - nval.ND) + nx);
      auto s1 = std::min(state.ST + std::abs(val.ST - nval.ST) + nx,
                         state.ND + std::abs(val.ND - nval.ST) + nx);
      val = nval;
      state = MP(s0, s1);
    }
    return std::min(state.ST, state.ND);
  }
  int solve_dumb() { return {}; }
};
