...
template<class T> T checkmin(T &a, T b){if (b<a)a=b; return a;}
template<class T> T checkmax(T &a, T b){if (b>a)a=b; return a;}
...
struct SolverContext {
  ...
  int solve() {
    ...
    l[0] = 0;
    REP (i, n)
      l[i + 1] = l[i] + (tb[i] >= checkmax(m ,tb[i]));

    r.resize(n+2, 0);
    m = -1;
    r[n] = 0;
    REPV (i, n)
      r[i] = r[i+1] + (tb[i] >= checkmax(m, tb[i]));

    int res = 0;
    REP (i, n+1)
      checkmax(res, l[i] + r[i]);
