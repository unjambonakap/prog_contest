...
int inv(s64 a) { return faste(a % mod, mod - 2); }
s64 dp[maxn][maxn];
s64 fact[maxn2 + 1];
s64 ifact[maxn2 + 1];
...
struct SolverContext {
  ...
  s64 go(int in, int ik) {
    ...
    int rem = (n - in);
    if (r != -1) return r;
    r = go(in + 1, ik) * in * rem % mod * fact[m - ne - 1] % mod * ifact[m - ne - 1 - (in - 1)] +
        go(in + 2, ik + 1) * rem * (rem - 1) / 2 % mod * fact[m - ne - 1] % mod *
          ifact[m - ne - 1 - 2 * in];
    r %= mod;
  ...
  s64 ank(int n, int k) { return fact2(n) * inv(fact2(n - k)) % mod; }
  s64 cnk(int n, int k) { return n < k || k < 0 ? 0: fact2(n) * inv(fact2(k) * fact2(n - k) % mod) % mod; }
  ...
  int solve2() {
    ...
    FOR (i, 1, n / 2 + 1) {
      ...
      int se = 2 * xb;
      tote += se + 1;
      // because indepence of events:
      // A: pi > pj, j<i
      // B: pk, pj, j<k
      cur = cur * inv(tote) % mod;
      if (i >= k) {
        s64 num_match_i = ank(n, 2 * i) % mod * inv(faste(2, i)) % mod;
        res += sgn * cur * num_match_i % mod * cnk(i, k) % mod;
        res %= mod;
      }
    }
    // res = res * inv(fact2(m)) % mod;
    res = (res % mod + mod) % mod;
    return res;
  }
  int solve_dumb() {
    ...
    FOR (i, 1, n / 2 + 1) {
      ...
      int xb = n - 2 * i;
      // cur = cur * inv(tote) % mod;
      cur = cur * inv((mod + cnk(n, 2) - cnk(xb, 2)) % mod) % mod;
      if (i >= k) {
        s64 num_match_i = ank(n, 2 * i) % mod * inv(faste(2, i)) % mod;
        res += sgn * cur * num_match_i % mod * cnk(i, k) % mod;
        res %= mod;
  ...
  int solve() {
    ...
    OPA_DISP0(go(0, 0), fact[m]);
    return go(0, 0) * inv(fact[m]) % mod;
  }
};
...
int main() {
  ...
  int tn;
  fact[0] = 1;
  REP (i, maxn2) fact[i + 1] = fact[i] * (i + 1) % mod;
  REP (i, maxn2 + 1) ifact[i] = inv(fact[i]);
