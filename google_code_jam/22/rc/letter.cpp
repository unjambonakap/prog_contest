...
struct SolverContext {
  ...
  std::string solve() {
    ...
    REP (i, n) {
      ...
      FOR(j,1,tb[i].size()-1)
        if (tb[i][j]!=tb[i].back() && tb[i][j]!=tb[i][0]) pre[tb[i][j]] = -1;
    }
    std::string res;
    std::stack<char> q;
    std::set<int> seen;
    ...
    while (q.size()) {
      int a = q.top();
      q.pop();
      if (!mp[a].size()) continue;
      vi after;
      for (auto &x : el[a]) {
        after.pb(x);
        seen.insert(x);
        if (!--pre[tb[x].back()]) q.push(tb[x].back());
      }

      for (auto &u : mp[a])
        if (seen.insert(u).second) res += tb[u];
      for (auto x : after){
        res += tb[x];
      }
    }
    ...
    for (auto c : res) {
      if (last != c) {
        last = c;
        if (!vv.insert(last).second) return {};
      }
    }
