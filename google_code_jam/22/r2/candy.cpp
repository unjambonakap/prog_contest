...
struct SolverContext {
  typedef std::vector<pii> res_t;
  int n;
  std::vector<Point<s64> > pa;
  Point<s64> tg;
  std::vector<Point<s64> > pb;
  std::vector<std::vector<s64> > dist;
  std::vector<vi> el;
  std::vector<std::set<std::pair<s64, int> > > edges;
  ...
  void read() {
    if (TEST_RAND) {
      ...
      pa.resize(n);
      pb.resize(n);
      REP (i, n) g_reader.readline() >> pa[i].x >> pa[i].y;
      g_reader.readline() >> tg.x >> tg.y;
      REP (i, n) g_reader.readline() >> pb[i].x >> pb[i].y;
    }
  }
  ...
  res_t solve() {
    res_t res;
    dist.resize(n, std::vector<s64>(n, -1));
    el.resize(n);
    edges.resize(n);
    REP (i, n) {
      s64 dx = (pa[i] - tg).dot(pa[i] - tg);

      REP (j, n) {
        s64 d = (pa[i] - pb[j]).dot(pa[i] - pb[j]);
        if (d <= dx) {
          dist[i][j] = d;
          el[i].pb(j);
          edges[i].insert(MP(d, j));
        }
      }
    }
    ...
    while (rem.size() != 0) {
      ...
      while (seen.insert(x).second) {
        x = match[edges[x].begin()->ND];
      }

      int ox = x;
      do {
        int m = edges[x].begin()->ND;
        int nx = match[m];
        match[m] = x;
        m2[x] = m;
        x = nx;
      ...
      rem.erase(x);
      int m = edges[x].begin()->ND;
      res.emplace_back(x, m);
      for (auto y : rem) edges[y].erase(MP(dist[y][m], m));
    }
