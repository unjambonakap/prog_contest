...
struct SolverContext {
  ...
  pii next(pii p) {
    if (std::abs(p.ST) < std::abs(p.ND)) {
      return p.ND > 0 ? p + MP(1, 0) : p - MP(1, 0);
    }
    return p.ST > 0 ? p + MP(0, 1) : p - MP(0, 1);
  }
  ...
  pii num2pos(s64 v) {
    REP (i, n) {
      ...
      if (v < 4 * shell - 4) {
        v -= shell - 1;
        if (v < 0) return MP(cx + v, -cx);
        v -= shell - 1;
        if (v < 0) return MP(cx, cx + v);
        v -= shell - 1;
        if (v < 0) return MP(-cx - v, cx);
        v -= shell - 1;
        return MP(-cx, -cx - v);
  ...
  s64 num(pii p) {
    s64 u = std::max(std::abs(p.ST), std::abs(p.ND));
    s64 u2 = 2 * u;
    s64 v = n * n - (2 * u + 1) * (2 * u + 1);
    if (p.ND == -u)
      v += p.ST + u;
    else if (p.ST == u)
      v += 1 * u2 + u + p.ND;
  ...
  const int vx[5] = { -1, 0, 1, 0, -1 };

  std::vector<pii> build_desc(const std::vector<pii> &tb) {
    std::vector<pii> res;
    for (int i = 1; i < tb.size(); ++i) {
      s64 a = num(tb[i - 1]);
      s64 b = num(tb[i]);
      if (a + 1 != b) res.emplace_back(a, b);
    }
    return res;
  }

  int get_min(pii p) { return std::abs(p.ST) + std::abs(p.ND); }
  int sgn(int x) { return x > 0 ? 1 : -1; }

  std::vector<pii> solve() {
    // return solve_dumb();
    int minv = n - 1;
    // REP (i, n * n) assert(num(num2pos(i)) == i);
    if (k < minv || (k - minv) % 2 != 0) return {};
    ...
    move.pb(px);
    while (px != MP(0, 0)) {
      pii pa = px + MP(-sgn(px.ST), 0);
      pii pb = px + MP(0, -sgn(px.ND));
      if (px.ST != 0 && get_min(pa) == get_min(px) - 1 && num(pa) > num(px))
        px = pa;
      else
        px = pb;
      move.pb(px);
    ...
    auto res = build_desc(move);
    s64 cnt = n * n-1;
    REP (i, res.size()) cnt -= res[i].ND - res[i].ST - 1;
    assert(cnt == k);
    return res;
  }
  ...
  std::vector<pii> solve_dumb() {
    ...
    while (!q.empty()) {
      pii e = q.front();
      q.pop();
      pii p = num2pos(e.second);
      if (e.first == k && p == MP(0, 0)) {
        std::vector<pii> res;
        res.emplace_back(num2pos(e.second));
        while (par.count(e)) {
          pii prev = par[e];
          res.emplace_back(num2pos(prev.second));
          e = prev;
        }
        std::reverse(ALL(res));
        return build_desc(res);
      }

      REP (k, 4) {
        pii np = p + MP(vx[k], vx[k + 1]);
        if (std::max(std::abs(np.ST), std::abs(np.ND)) > n2) continue;
        int nx = num(np);
        pii ne = MP(e.first + 1, nx);
        if (nx > e.second && par.emplace(ne, e).second) q.push(ne);
      }
    }
    return {};
  }
