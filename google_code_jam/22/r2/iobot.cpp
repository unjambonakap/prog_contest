...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      REP (i, nx) {
        int x = rng() % 20;
        int y = rng() % 2;
        if (tx.insert(x).second) {
          tb.emplace_back(x, 2 * y - 1);
        }
      }
      n = tb.size();

    } else {
      g_reader.readline() >> n >> s;
      REP (i, n) {
        int a, type;
        g_reader.readline() >> a >> type;
        type = type * 2 - 1;
        tb.emplace_back(a, type);
      }
    }
    std::sort(ALL(tb));
  }
  ...
  res_t solve1() {
    std::vector<int> data;
    std::vector<s64> csum[2];
    int nx = tb.size();
    REP (f, 2) {
      csum[f].resize(nx + 1, 0);
      REP (i, nx) csum[f][i + 1] = csum[f][i] + (tb[i].ND == 2 * f - 1) * tb[i].ST;
    }

    data.resize(nx * 2 + 1, 0);
    std::vector<s64> best(nx + 1, 1e18);
    int mid = tb.size();
    ...
    FOR (i, 1, tb.size()) {
      auto &r = best[i + 1];
      int cx = 2 * tb[i].ST;
      checkmin(r, best[i] + cx);
      checkmin(r, cx + (tb[i].ND == tb[i - 1].ND) * s + best[i - 1]);
      cp += tb[i].ND;

      if (tb[i].ND != tb[i - 1].ND)
        ;
      else {
        int tf = tb[i].ND == 1;
        int fp = data[mid + cp];
        checkmin(r, best[fp] + 2 * (csum[tf][i + 1] - csum[tf][fp]));
      }
      data[mid + cp] = i + 1;
    }
  ...
  res_t solve() {
    ...
    for (int f = -1; f <= 1; f += 2) {
      ...
      for (auto x : orig)
        if (f * x.ST > 0) tb.pb(MP(x.ST * f, x.ND));
      if (f == -1) std::reverse(ALL(tb));
      res += solve1();
    }
  ...
  res_t solve1_dumb() {
    std::unordered_map<int, s64> state;
    state[0] = 0;
    for (auto e : tb) {
      std::unordered_map<int, s64> ns;
      auto add = [&](int a, s64 v) {
        if (!ns.count(a) || ns[a] > v) ns[a] = v;
      };
      ...
      for (auto s : state) {
        ...
        add(s.ST, s.ND + cx);
        if (e.ND * s.ST <= -1)
          add(s.ST + e.ND, s.ND + cx);
        else
          add(s.ST + e.ND, s.ND);
        if (e.ND * s.ST >= 1) add(s.ST - e.ND, s.ND + cx + this->s);
      }
