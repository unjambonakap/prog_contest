...
struct Graph {
  ...
  int dfs(int a, int p = -1) {
    if (visid[a] != -1) return visid[a];
    int pos = stk.size();
    stk.pb(a);
    int res = visid[a] = id++;
    for (auto x : el[a]) {
      if (x == p) continue;
      checkmin(res, dfs(x, a));
    }
    if (res < visid[a]) return res;

    vi tb = vi(stk.begin() + pos, stk.end());
    stk.resize(pos);
    for (auto x : tb) v2c[x] = cc.size();
    cc.pb(tb);

    return res;
  }

  void compute_2cc(int sp) {
    v2c.resize(n);
    visid.resize(n, -1);
    id = 0;
    dfs(sp);
...
struct SolverContext {
  ...
  res_t solve() {
    ...
    REP (i, n)
      if (d0[i] < oo) can.pb(i);
    OPA_DISP0(g.cc, g.v2c);
    for (auto x : can) {
      if (d0[x] < d1[x]) {
        if (g.cc[g.v2c[x]].size() > 2) return -1;
        checkmax(res, d1[x]);
      }
    }
