...
s64 faste(s64 a, int p) {
  s64 x = 1;
  for (; p > 0; p >>= 1, a = 1ull * a * a % mod)
    if (p & 1) x = 1ll * x * a % mod;
  return x;
}

s64 inv(s64 x) { return faste(x, mod - 2); }
s64 negp(s64 x) { return (mod + 1 - x) % mod; }

const s64 i2 = inv(2);
const s64 modp1 = mod + 1;

struct SolverContext {
  typedef s64 res_t;
  vi next;
  std::vector<std::set<int> > prevs;
  vi type;
  int n;
  int splitp;
  int state_after;
  int next_split_state;
  ...
  void read() {
    if (TEST_RAND) {
      ...
      prevs.resize(n);
      next.resize(n);
      REP (i, n) type[i] = rng() % 3 - 1;
      REP (i, n) next[i] = (rng() % (n - 1) + i + 1) % n;
    ...
    REP (i, n) prevs[next[i]].insert(i);
  }

  s64 prob_filled_after_action(int node) {
    if (node == splitp) return state_after;
    assert(next[node] < node);
    s64 pafter = prob_filled_after_action(next[node]);
    return prob_filled_at(node) *
           negp(negp(pafter) * negp(prob_filled_between(next[node], next[node], node - 1)));
  }

  s64 prob_filled_at(int node) {
    if (node == next[splitp]) {
      return prob_filled_between(next[splitp], splitp + 1, next[splitp], next_split_state);
    }
    if (type[node] == 1) return 1;
    s64 vp0 = prob_filled_between(node, 0, node);
    if (type[node] == 0) return vp0;
    return negp(i2 * negp(vp0));
  }

  s64 prob_filled_between(int node, int t1, int t2, int use_state = -1) {
    if (use_state != -1 && use_state) return 1;
    s64 notfilled_between = 1;
    for (auto x : prevs[node]) {
      if (x >= t1 && x <= t2) {
        notfilled_between = notfilled_between * (modp1 - prob_filled_at(x)) % mod;
      }
    }
    return negp(notfilled_between);
  }

  res_t solve() {
    int x = n - 1;
    std::set<int> seen;
    while (seen.insert(x).second) x = next[x];
    seen.clear();
    while (seen.insert(x).second) x = next[x];
    x = n - 1;
    while (!seen.count(next[x])) x = next[x];
    splitp = next[x];

    x = n - 1;
    while (!(next[x] < x && next[next[x]] > next[x])) x = next[x];
    splitp = next[x];

    splitp = next[x];
    int cntx = 0;
    res_t res = 0;
    int otype = type[splitp];
    int otype_next = type[next[splitp]];
    prevs[next[splitp]].erase(splitp);
    vi otyp = type;

    REP (statep, 2)
      REP (state_nextp, 2) {
        if (otype != -1 && otype != statep) continue;
        if (otype_next != -1 && otype_next != state_nextp) continue;
        type[splitp] = statep;
        type[next[splitp]] = state_nextp;

        REP (state_before, 2)
          REP (next_split_state_before, 2) {
            this->state_after = state_before && next_split_state_before;
            this->next_split_state = state_before || next_split_state_before;
            // assume at time=splitp, splitp has value f
            /*
               -> prob that n-1 is filled : backtracking + when encountering splitp, use given
               value
               -> prob that next[n-1] is filled at time n-1;


               */

            s64 pnext_filled = prob_filled_between(next[splitp], 0, splitp - 1, type[next[splitp]]);
            s64 p_filled = prob_filled_at(splitp);

            s64 px = 1;
            px = px * (state_before ? p_filled : negp(p_filled)) % mod;
            px = px * (next_split_state_before ? pnext_filled : negp(pnext_filled)) % mod;
            if (px == 0) continue;
            s64 nextn1_filled =
              negp(negp(prob_filled_after_action(next[n - 1])) *
                   negp(prob_filled_between(next[n - 1], next[n - 1], n - 2)) % mod);
            s64 cur = px * prob_filled_at(n - 1) % mod * nextn1_filled % mod;
            res = res + cur;
          }
      }
    res = (res%mod+mod)%mod;
    ...
    REP (x, n)
      if (x != splitp) cntx += type[x] == -1;
    type = otyp;
    return faste(2, cntx) * res % mod;
  }

  bool simulate(vi state) {
    REP (i, n) {
      if (state[i] && !state[next[i]]) {
        state[i] = 0;
        state[next[i]] = 1;
      }
    }
  ...
  res_t solve_dumb() {
    ...
    vi cur = type;
    res_t res = 0;
    for (s64 i = 0; i < 1ull << tg.size(); ++i) {
      REP (j, tg.size()) cur[tg[j]] = i >> j & 1;
      if (simulate(cur)) {
        ++res;
      }
    }
  ...
  void disp(const res_t &res) { printf("%Ld\n", res); }
};
