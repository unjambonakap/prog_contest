...
struct SolverContext {
  ...
  std::unordered_map<int, pii> t2mxy;
  std::vector<std::map<int, vi> > t2s;
  std::set<int> tl;
  vi ducks;
  graph::Graph duckgraph;

  bool conflict(int dt, int dx, int dy) { return 1ull * dt * dt < 1ll * dx * dx + 1ll * dy * dy; }

  std::unordered_set<int> mark_conflicting(int id, int t, int x, int y, int sid = -1) {

    auto &e = t2s[id];
    auto it0 = e.lower_bound(t);
    vi thrash;
    REP (f, 2) {
      auto it = it0;
      if (f) {
      ...
      while (it != e.end()) {
        ...
        auto &sx = sl[it->second[0]];
        if (conflict(t - sx.t, x - sx.x, y - sx.y)) {
          thrash.pb(sx.t);
        } else
          break;

        if (f && it == e.begin()) break;
        if (f)
          --it;
        else
          ++it;
    ...
    for (auto ti : thrash) {
      for (auto ix : e[ti]) {
        if (sid != -1) ducks[sl[ix].a] = 1;
        marked.insert(sl[ix].a);
      }
      e.erase(ti);
    }
    if (sid != -1) {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      g_reader.readline() >> n >> m >> s;
      sl.resize(s);
      REP (i, m) {
        int x, y, t;
        g_reader.readline() >> x >> y >> t;
        tl.insert(t);
        t2mxy[t] = MP(x, y);
      }
      REP (i, s)
        g_reader.readline() >> sl[i].a >> sl[i].b >> sl[i].x >> sl[i].y >> sl[i].t, --sl[i].a,
          --sl[i].b;
    }
  }

  void meeting_conflict(int id, int tid, int tm) {
    auto &l = t2s[id][tid];
    auto &p = sl[l[0]];
    auto &xy = t2mxy[tm];
    if (!conflict(p.t - tm, p.x - xy.first, p.y - xy.second)) return;
    for (auto e : l) {
      duckgraph.adde(id, sl[e].a);
    }
  }

  void maybe_duck(int id) {
    auto &e = t2s[id];
    for (auto u : e) {
      auto it = tl.lower_bound(u.first);
      if (it != tl.end()) meeting_conflict(id, u.first, *it);
      if (it == tl.begin()) continue;
      --it;
      meeting_conflict(id, u.first, *it);
    }
  }
  ...
  res_t solve() {
    ...
    REP (si, s) {
      auto &s = sl[si];
      mark_conflicting(s.a, s.t, s.x, s.y, si);
      mark_conflicting(s.b, s.t, s.x, s.y, si);
    }

    duckgraph.setup_digraph(n);
    REP (i, n) maybe_duck(i);

    auto scc = graph::SCCDecomp();
    scc.setup(&duckgraph);
    ...
    if (stk.size() != 0) {
      while (stk.size()) {
        int a = stk.back();
        stk.pop_back();
        for (auto b : duckgraph.next(a))
          if (done.insert(b).second) stk.pb(b);
      }
      res = done.size();

    } else {
      res = INT_MAX;
      REP (i, scc.sccgraph.n())
        if (scc.sccgraph.next(i).size() == 0) checkmin<int>(res, scc.scc[i].size());
    }
    return res;
  }

  res_t solve_dumb() { return {}; }
  void disp(const res_t &res) { printf("%d\n", res); }
};
