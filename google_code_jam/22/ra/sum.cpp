...
#define OPA_DISPL0(...)                                                        \
  (std::cerr << OPA_LINE_INFO << ": " << RAW_OPA_DISP_VARS(__VA_ARGS__)        \
             << std::endl)
#define OPA_DISPL(msg, ...)                                                    \
  (std::cerr << OPA_LINE_INFO << "msg:" << msg << ": "                         \
             << RAW_OPA_DISP_VARS(__VA_ARGS__) << std::endl)

#define OPA_STREAM_STR(a)                                                      \
  ((static_cast<std::ostringstream &>(std::ostringstream() << a)).str())

#define OPA_DISP_VARS1(a) (std::cerr << _OPA_DISP_VAR1(a) << std::endl)
#define OPA_DISP_VARS2(a, b) (std::cerr << _OPA_DISP_VAR2(a, b) << std::endl)
#define OPA_DISP_VARS3(a, b, c)                                                \
  (std::cerr << _OPA_DISP_VAR3(a, b, c) << std::endl)
#define OPA_DISP_VARS4(a, b, c, d)                                             \
  (std::cerr << _OPA_DISP_VAR4(a, b, c, d) << std::endl)
#define OPA_DISP_VARS5(a, b, c, d, e)                                          \
  (std::cerr << _OPA_DISP_VAR5(a, b, c, d, e) << std::endl)
#define OPA_DISP_VARS6(a, b, c, d, e, f)                                       \
  (std::cerr << _OPA_DISP_VAR6(a, b, c, d, e, f) << std::endl)
#define OPA_DISP_VARS7(a, b, c, d, e, f, g)                                    \
  (std::cerr << _OPA_DISP_VAR7(a, b, c, d, e, f, g) << std::endl)
#define OPA_DISP_VARS8(a, b, c, d, e, f, g, h)                                 \
  (std::cerr << _OPA_DISP_VAR8(a, b, c, d, e, f, g, h) << std::endl)
#define OPA_DISP_VARS9(a, b, c, d, e, f, g, h, i)                              \
  (std::cerr << _OPA_DISP_VAR9(a, b, c, d, e, f, g, h, i) << std::endl)
#define OPA_DISP_VARS10(a, b, c, d, e, f, g, h, i, j)                          \
  (std::cerr << _OPA_DISP_VAR10(a, b, c, d, e, f, g, h, i, j) << std::endl)
...
struct SolverContext {
  ...
  pii find(int diff) {
    for (int i = 1;; ++i) {
      if (!used.count(i) && !used.count(i + diff)) {
        used.insert(i), used.insert(i + diff);
        return MP(i, i + diff);
      }
    }
  }
  ...
  void read() {
    if (TEST_RAND)
      ...
      g_reader.readline() >> n;
      REP (i, n/2-pw){
        fill.emplace_back(2*i+1, 2*i+2);
        used.insert(2*i+1);
        used.insert(2*i+2);
      }

      REP (i, pw) {
        auto r = find(1 << (pw-1-i));
        a.pb(r);
      }
  ...
  void solve() {
    ...
    vi res;
    REP (i, cnds.size() / 2)
      res.pb(cnds[2 * i + 1]), diff += cnds[2 * i + 1] - cnds[2 * i];
    REP (i, pw) {
      int v = 1 << (pw - 1 - i);
      if (diff > 0)
        diff -= v, res.pb(a[i].ST);
      else
        diff += v, res.pb(a[i].ND);
    }
    int pos = 0;
    if (diff % 2 == 1) {
      pos += 2, res.pb(fill[0].ST), res.pb(fill[0].ND);
      diff -= 3;
    }
    FOR (j, pos, fill.size()) {
      if (diff > 0)
        res.pb(fill[j].ST), --diff;
      else
        res.pb(fill[j].ND), ++diff;
    }
