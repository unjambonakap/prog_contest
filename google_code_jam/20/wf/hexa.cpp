...
#define two_pow64(x) (32-__builtin_clzll(x))
#define pb push_back
#define ST first
#define ND second
...
struct SolverContext {
  ...
  int s, e;
  int hex2i(char x) { return (x >= 'A' ? x - 'A' + 10 : x - '0'); }
  int norm(const std::string &s) {
    int res = 0;
    for (auto x : s) res = res << 4 | hex2i(x);
    return res;
  }
  ...
  std::unordered_map<s64, s64> hashm[maxd];
  s64 perm[maxb];
  struct HashBuilder {
    s64 h;
    s64 cur = 1;
    ...
    s64 trypush(int a) {
      int v = mp[a] == -1 ? cnt : mp[a];
      return v | (h << two_pow32(cnt + 1));
    }
  ...
  s64 f(int lim) {
    ...
    REP (v1, mv) {
      ...
      REP (j, d) {
        int dx = tg >> (4 * (d - 1 - j)) & 0xf;
        REP (k, dx)
          res += hashm[j + 1][h.trypush(k)];
        h.push(dx);
      }
      res += hashm[d][h.h];
    }
  ...
  frac<s64> solve() {
    ...
    REP (i, n)
      REP (j, n) {
        if (i == j) continue;
        HashBuilder hb;
        std::string b = tb[i] + tb[j];
        REP (k, b.size()) {
          int nx = k - d + 1;
          hb.push(hex2i(b[k]));
          if (nx >= 0) hashm[nx][hb.h] += perm[16 - hb.cnt];
        }
      }

    s64 q = n * (n - 1) * perm[16];
    int of = 1 << (4 * d);
    s64 p = f(e) - f(s - 1) + f(of + e) - f(of + s - 1);
    return frac<s64>(p, q);
  }

  frac<s64> solve_dumb() { return {}; }
};
