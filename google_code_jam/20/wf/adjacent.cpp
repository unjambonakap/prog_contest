...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      REP (i, n)
        rem1.pb(i), rem2.pb(i);
      std::shuffle(ALL(rem1), rng);
      std::shuffle(ALL(rem2), rng);
      REP (i, n)
        tile_and_pos.emplace_back(rem1[i], rem2[i]);

    } else {
      readline() >> n;
      tile_and_pos.resize(n);
      REP (i, n)
        readline() >> tile_and_pos[i].ST >> tile_and_pos[i].ND;
      REP (i, n)
        tile_and_pos[i] = tile_and_pos[i] - MP(1, 1);
    }
  }

  std::vector<int> state;
  std::set<int> rem;
  std::unordered_map<vi, int> dp1[maxn][maxn][2];
  std::unordered_map<vi, int> dp2[maxn][maxn][2];
  void tryerase(std::multiset<int> &ms, int key) {
    auto it = ms.find(key);
    if (it != ms.end()) ms.erase(it);
  }
  int comp1_break(int c1, int c2, bool player, int breakv, bool use_c2, std::multiset<int> &ms) {

    tryerase(ms, breakv);
    std::multiset<int> pending_ms;
    REP (p, breakv) {
      auto &t = use_c2 ? pending_ms : ms;
      t.insert(p);
      t.insert(breakv - p - 1);
      if (!rec1(c1, c2, ms, pending_ms, player ^ 1)) return 1;
      tryerase(t, p);
      tryerase(t, breakv - p - 1);
    }
    ms.insert(breakv);
    return 0;
  }

  int comp1(int c1, int c2, const vi &vals, const vi &pending, bool player) {

    std::multiset<int> ms(ALL(vals));
    std::multiset<int> empty_ms;

    for (auto v : vals) {
      for (auto x : pending) ms.insert(x);
      if (comp1_break(c1 - 1, c2, player, v, false, ms)){
        return 1;
      }
      for (auto x : pending) tryerase(ms, x);
    }

    if (pending.size()) {
      if (pending.size() == 1) {
        ms.insert(pending[0] - 1);
        if (!rec1(c1 + 1, c2 - 1, ms, empty_ms, player ^ 1)) return 1;
        tryerase(ms, pending[0] - 1);
      }
      for (auto x : pending) ms.insert(x);
      for (auto x : pending)
        if (x > 1 && comp1_break(c1 - 1, c2, player, x, false, ms)) return 1;
      return 0;
    }

    if (pending.size() == 2) {
      // have to use one of c1, forced
      return 0;
    }

    for (auto v : vals) {
      if (comp1_break(c1 + 1, c2 - 1, player, v, true, ms)) return 1;
    }

    return 0;
  }

  int rec1(int c1, int c2, std::multiset<int> &vals, std::multiset<int> &pending, bool player) {
    if (c1 < 0 || c2 < 0) return true;
    if (c2 == 0 && c1 == 0) return player;
    vals.erase(0);
    pending.erase(0);
    vi tb(ALL(vals)), tb_pending(ALL(pending));

    if (pending.size() == 0 && c2 == 0) return player;
    if (pending.size() && !player) return 1;

    auto &mp = dp1[c1][c2][player];
    auto tbser = tb;
    for (auto x : pending) tbser.pb(-x);

    if (mp.count(tbser)) return mp[tbser];
    std::set<int> ns;
    auto v = comp1(c1, c2, tb, tb_pending, player);
    //OPA_DISP0(c1, c2, tb, tb_pending, player, v);
    return mp[tbser] = v;
  }

  bool is_winning(int pl) {
    REP (i, n - 1)
      if (std::abs(state[i] - state[i + 1]) == 1) return 1 ^ pl;
    if (rem.size() == 0) return pl;

    if (pl) {
      REP (i, n) {
        if (state[i] >= 0) continue;
        std::vector<int> rem_lst(ALL(rem));
        for (auto x : rem_lst) {
          rem.erase(x);
          state[i] = x;
          bool rv = is_winning(0);
          state[i] = -2;
          rem.insert(x);
          if (!rv) {
            return true;
    ...
    REP (i, n) {
      if (((i > 0 && state[i - 1] < 0) || (i + 1 < n && state[i + 1] < 0)) &&
          (rem.count(state[i] - 1) || rem.count(state[i] + 1)))
        return true;
    }

    std::multiset<int> vals, slots;
    REP (i, n) {
      if (state[i] >= 0) continue;
      int c = 0;
      while (i < n && state[i] < 0) ++i, ++c;
      slots.insert(c);
    }
    ...
    REP (i, n) {
      ...
      int c = 0;
      while (i < n && rem.count(i)) ++i, ++c;
      vals.insert(c);
    }
    std::multiset<int> empty_ms;
    int max_slot = *slots.rbegin();
    int max_val = *vals.rbegin();
    if (max_slot >= 3 && max_val >= 3) return true;
    if (max_slot <= 2) {
      int cnt[2] = { 0, 0 };
      for (auto x : slots) cnt[x - 1]++;
      return rec1(cnt[0], cnt[1], vals, empty_ms, 0);
    } else {
      int cnt[2] = { 0, 0 };
      for (auto x : vals) cnt[x - 1]++;
      return rec1(cnt[0], cnt[1], slots, empty_ms, 0);
    }
    return false;
  }
  ...
  pii solve() {
    ...
    bool winning = true;
    REP (i, n) {
      state[tile_and_pos[i].ND] = tile_and_pos[i].ST;
      rem.erase(tile_and_pos[i].ST);
      bool nwin = is_winning(i + 1 & 1);
      if (0 && nwin != is_winning_dumb(state, i + 1 & 1)) {
        OPA_DISP0(nwin, state, i + 1 & 1);
        assert(0);
      }

      if (winning && nwin) (i & 1 ? res.ND : res.ST)++;
      winning = nwin;
    }
    return res;
  }

  std::unordered_map<vi, int> dp_dumb[2];
  bool is_winning_dumb(const vi &state, int player) {
    REP (i, state.size() - 1)
      if (std::abs(state[i] - state[i + 1]) == 1) return !player;
    std::set<int> rem;
    REP (i, n)
      rem.insert(i);
    for (auto x : state) rem.erase(x);
    if (rem.size() == 0) return player;

    auto &r = dp_dumb[player];
    if (r.count(state)) return r[state];
    vi ns = state;
    ...
    REP (i, ns.size()) {
      ...
      for (auto y : rem) {
        ns[i] = y;
        if (!is_winning_dumb(ns, player ^ 1)) goto win;
      }
      ns[i] = -2;
    }
...
int main() {
  ...
  REP (ti, tn) {
    ...
    if (0) {
      ...
      ms1.insert(3);
      ms1.insert(1);
      OPA_DISP0(sc.rec1(2, 1, ms1, ms2, false));
      break;
    }
    if (TEST_CHECK) {
      auto r1 = sc.solve();
