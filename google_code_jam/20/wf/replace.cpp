...
struct SolverContext {
  ...
  void read() {
    ...
    if (TEST_RAND) {
      ...
      checkmin(m, 13);
      REP (i, n)
        if (rng() % 10 != 0) seen.insert(i);
      REP (i, m) {
        while (true) {
          int a = rng() % n;
          int b = rng() % n;
          if (a == b) continue;
          if (edges.emplace(a, b).second) break;
        }
      }
    } else {
      std::string s;
      int ne;
      g_reader.readline() >> s >> ne;
      ...
      REP (i, ne) {
        ...
        edges.emplace(rmp[sx[0]], rmp[sx[1]]);
      }
      n = rmp.size();
    }
    graph.setup_digraph(n);
  ...
  int solve() {
    ...
    REP (scid, nscc) {
      ...
      if (full) {
        int idx = sccflow.adde(sccflow.S, 2 * scid + 1, 0, 1);
        --have;
  ...
  static constexpr int maxn = 11;
  static constexpr int maxm = 11;
  int n, m;

  int solve_dumb() {
    int state = 0;
    m = graph.el.size();
    for (auto x : seen) state |= 1 << x;
    ...
    while (q.size() > 0) {
      ...
      if (x.second == (1 << m) - 1) {
        checkmax(res, count_bit(x.first));
        continue;
      }
      ...
      REP (i, m) {
        int nx = x.first;
        auto &e = graph.el[i];
        if (nx >> e.from & 1) nx ^= 1 << e.from, nx |= 1 << e.to;
        int ne = x.second | 1 << i;
        if (seen.emplace(nx, ne).second) q.emplace(nx, ne);
      }
    }
    return res;
  }
  ...
  REP (ti, tn) {
    ...
    if (TEST_CHECK) {
      ...
      if (r1 != r2) {
        ...
        OPA_DISP0(r1, r2);
        puts("");
        for (auto e :sc.graph.el) OPA_DISP0(e.from, e.to);
        assert(r1 == r2);
      }
    } else {
