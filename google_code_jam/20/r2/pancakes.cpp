...
struct SolverContext {
  ...
  std::tuple<s64, s64, s64> solve() {
    s64 l = _l;
    s64 r = _r;
    s64 diff = std::abs(l - r);

    s64 n = std::max<s64>(std::sqrt(diff * 2) - 20, 0);
    if (l < r)
      r -= n * (n + 1) / 2;
    else
      l -= n * (n + 1) / 2;
    ...
    s64 target = std::max(n, T - 10);

    s64 c1 = (target - n + 1) / 2;
    s64 c2 = (target - n) / 2;
    s64 x1 = n * c1 + (c1 * (c1 - 1));
    s64 x2 = (n + 1) * c2 + (c2 * (c2 - 1));
    if (l >= r) {
      l -= x1;
      r -= x2;
    } else
  ...
  std::tuple<s64, s64, s64> solve_dumb() {
...
int main() {
  ...
  REP (ti, tn) {
    ...
    if (TEST_CHECK) {
      ...
      printf("Case #%d: ", ti + 1);
      auto res = sc.solve();
      printf("%Ld %Ld %Ld", std::get<0>(res), std::get<1>(res), std::get<2>(res));
      puts("");
    }
  }
