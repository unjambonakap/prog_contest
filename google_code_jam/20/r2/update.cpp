...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      REP (i, ne) {
        while (true) {
          int a = rng() % (n-1) + 1;
          int b = rng() % a;
          if (seen.emplace(b, a).second) {
            break;
          }
        }
      }

      edges.insert(edges.end(), ALL(seen));
      xc.resize(n);
    ...
    if (TEST_RAND) {
      ...
      while (!q.empty()) {
        ...
        q.pop();
        int a = p.second;
        if (-p.ST != dist[a]) continue;
        for (auto b : el[a]) {
          int nc = dist[a] + rng() % mc + 1;
          if (dist[b] > nc) {
            dist[b] = nc;
            q.emplace(-nc, b);
          }
  ...
  std::vector<int> solve() {
    ...
    while (true) {
      ...
      std::vector<int> p1;
      for (auto e : kc) {
        assert(e.first >= fixed);
        if (e.first == fixed)
          p1.pb(e.second);
        else
          break;
      }
      for (auto x : p1) kc.erase(MP(fixed, x));
      ...
      if (p1.size() == 0) {
        ...
        for (auto x : p1) if (res[x] == -1) res[x] = t, filter.pb(x);
        for (auto x : filter) push(x);
        ++t;
      }
    }
    ...
    REP (i, m) {
      ...
      int ta = res[e.first];
      int tb = res[e.second];
      reslat[i] = std::max(1, std::max(ta, tb) - std::min(ta, tb));
    }
    return reslat;
  }
