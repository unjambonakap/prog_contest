...
struct SolverContext {
  ...
  int go(std::map<int,int> tb) const {
    int res = 0;
    for (auto x : tb) if (x.first > 1) res += x.second * x.first;
    if (tb[1] == 1) ++res;
    else if (tb[1] > 1) res += (res & 1) ? 1 : 2;
    return res;
  }
  ...
  int solve() {
    ...
    REP (i, n)
      REP (j, i) {
        std::unordered_map<s64, vi> mp;
        REP (k, n) { mp[(points[k] - points[i]).cross(points[j] - points[i])].pb(k); }
        std::map<int, int> tb;
        for (auto &x : mp) ++tb[x.second.size()];
        checkmax(best, go(tb));
      }
    return best;
  }
