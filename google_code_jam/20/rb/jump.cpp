...
struct SolverContext {
  ...
  int rec(int pos, int fx, int fy, bool build) {
    int &r = dp[pos][fx + 1][fy + 1];
    if (!build) {
      if (r != -1) return r;
      r = inf;
    } else
      assert(r != inf);
    if (pos == maxn) return r;
    if (!fx && !fy && (std::max(x, y) >> pos) == 0) {
      r = pos;
      return r;
    }
    REP (k, 4) {
      int dx = k < 2 ? (k & 1) * 2 - 1 : 0;
      int dy = k >= 2 ? (k & 1) * 2 - 1 : 0;
      int nx = fx + dx - (x >> pos & 1) * sx;
      int ny = fy + dy - (y >> pos & 1) * sy;
      if (nx & 1) continue;
      if (ny & 1) continue;
      int nr = rec(pos + 1, nx / 2, ny / 2, false);
      if (build) {
        if (nr == r) {
          build_sol.pb(k);
          rec(pos + 1, nx / 2, ny / 2, true);
          break;
        }
      } else
        checkmin(r, nr);
  ...
  std::string solve() {
    ...
    int r =rec(0, 0, 0, false);
    if (r >= inf) return "IMPOSSIBLE";
    rec(0, 0, 0, true);
    for (auto x : build_sol) res.pb(mp[x]);
    return res;
  }
