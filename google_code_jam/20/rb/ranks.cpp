...
struct hash_t {
  template <typename T1> size_t operator()(const std::vector<T1> &x) const {
    size_t h = 0;
    REP (i, x.size())
      h = h * 31 + x[i];
    return h;
...
struct CostC {
  ...
  int compute_ncost(int l, int r) {
    bool rem = r + 1 < _cur.size();
    std::vector<std::tuple<u8, u8, bool> > ops;
    ops.pb(std::make_tuple(_cur[l], _cur[l + 1], false));
    if (rem) ops.pb(std::make_tuple(_cur[r], _cur[r + 1], false));
    ops.pb(std::make_tuple(_cur[r], _cur[0], true));
    if (rem) ops.pb(std::make_tuple(_cur[l], _cur[r + 1], true));
    for (auto &op : ops) update(std::get<0>(op), std::get<1>(op), std::get<2>(op));
    int res = cost;

    std::reverse(ALL(ops));
    for (auto &op : ops) update(std::get<0>(op), std::get<1>(op), std::get<2>(op) ^ 1);
    return res;
  }

  inline void update(u8 a, u8 b, bool add) {
    int dv = add ? 1 : -1;
  ...
  int compute(const std::vector<u8> &cur) {
    _cur = cur;
    cost = 0;
    memset(cnt, 0, sizeof(cnt));
    REP (i, cur.size() - 1) { update(cur[i], cur[i + 1], true); }
    return cost;
  }
};
...
struct SolverContext {
  ...
  std::vector<pii> solve() {
    ...
    while (cur != end) {
      ...
      REP (r, n)
        REP (l, r) {
          if (r != n) {
            if (cur[l] != cur[r + 1] && cur[l] != cur[r + 1] - 1) continue;
          }
          int ncx = cx.compute_ncost(l, r);
          if (ncx > best) best = ncx, play = MP(l, r);
          if (best == icost + 2) break;
        }
      res.pb(play);
      assert(icost < best);
      cur = apply(cur, play.ST, play.ND);
    }
    return res;
  }

  int cost(const std::vector<u8> &cur) const {
    int c = 0;
    ...
    REP (i, n - 1) {
      if (cur[i] == cur[i + 1])
        ...
        if (seen.insert(cur[i + 1]).second) ++c;
      }
    }
    return c;
  }

  std::vector<u8> apply(const std::vector<u8> &cur, int l, int r) const {
  ...
  std::vector<pii> solve_dumb() {
    std::unordered_map<std::vector<u8>, int, hash_t> seen;
    std::vector<u8> start;
    REP (j, s)
      REP (i, r)
        start.pb(i);
      ...
      REP (r, n)
        REP (l, r) {
          ...
          FOR (i, r + 1, n)
            nc.pb(cur[i]);
          assert(nc.size() == n);
          if (seen.insert(MP(nc, cx + 1)).second) q.push(nc);
        }
    }
  }
};
...
int main() {
  ...
  REP (ti, tn) {
    ...
    if (TEST_CHECK) {
      ...
      auto res = sc.solve();
      printf("%lu", res.size());
      for (auto x : res) printf("\n%d %d", x.ST + 1, x.ND - x.ST);
      puts("");
    }
  }
