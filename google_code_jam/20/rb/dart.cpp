...
struct Sim {
  int r, x, y;
  std::uniform_int_distribution<int> dis{ -maxx, maxx };
  std::uniform_int_distribution<int> dis_r{ maxx / 2, maxx };
  bool won = false;
  int remq = 300;
  void gen_rand() {
    r = dis_r(rng);
    std::uniform_int_distribution<int> dis2{ -maxx + r, maxx - r };
    x = dis2(rng);
    y = dis2(rng);
  }

  std::string query(int xq, int yq) {
    assert(remq > 0);
    --remq;
    s64 xd = xq - x;
    s64 yd = yq - y;
    ...
    return rx <= 1ull * r * r ? "HIT" : "MISS";
  }
  bool is_winning() const { return won; }
};
...
struct SolverContext {
  ...
  void solve() {
    while (true) {
      int x, y;
      x = sim.dis(rng), y = sim.dis(rng);
      if (query(x, y) == -1) continue;
      int lx = x;
      int hx = x;
