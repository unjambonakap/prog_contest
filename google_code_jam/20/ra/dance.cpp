...
struct SolverContext {
  ...
  inline int id(int x, int y) const { return x * ny + y; }
  int in(int a, int b) const { return a >= 0 && b >= 0 && a < nx && b < ny; }
  int get(int a, int k) const {
    if (a == -1) return a;
    if (!dead[a]) return a;
    return nodes[a].peers[k] = get(nodes[a].peers[k], k);
  }

  pii pos(int idx) const { return MP(idx / ny, idx % ny); }
  int peer(int idx, int k) const {
    auto p = pos(idx);
    p.ST += dd[k];
    p.ND += dd[k + 1];
    if (!in(p.ST, p.ND)) return -1;
    return id(p.ST, p.ND);
  }
  ...
  s64 solve() {
    ...
    while (true) {
      ...
      for (auto x : dying){
        REP(k,4) nrecomp.insert(get(x, k));
      }
      nrecomp.erase(-1);

      std::swap(recomp, nrecomp);
    }
    return res;
  }
  s64 solve_dumb() { return 0; }
};
