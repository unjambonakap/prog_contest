...
struct SolverContext {
  ...
  std::string solve() {
    ...
    REP (i, n) {
      REP (j, pos[i].ST) {
        if (res.size() <= j) res.pb(tb[i][j]);
        if (tb[i][j] != res[j]) goto fail;
      }
    }
    REP (i, n) {
      FOR (j, pos[i].ST, pos[i].ND)
        if (tb[i][j] != '*') res.pb(tb[i][j]);
    }

    REP (i, n) {
      FORV (j, pos[i].ND, tb[i].size()) {
        if (suffix.size() <= tb[i].size() - j - 1) suffix.pb(tb[i][j]);
        if (tb[i][j] != suffix[tb[i].size() - 1 - j]) goto fail;
      }
    }
    std::reverse(ALL(suffix));
    res += suffix;
