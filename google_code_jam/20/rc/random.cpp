...
struct SolverContext {
  ...
  std::string solve() {
    std::unordered_map<u8, int> cnt;
    REP (i, n)
      if (s[i].size() == u) cnt[s[i][0]]++;
    std::set<u8> z;
    for (auto x : s) z.insert(x.back());
    for (auto x : s)
      if (x.size() > 1) z.erase(x[0]);
    OPA_DISP0(z);
    assert(z.size() == 1);
    std::vector<std::pair<int, u8> > tb;
    for (auto x : cnt) tb.emplace_back(x.second, x.first);
    std::sort(ALL(tb));
    std::reverse(ALL(tb));
    std::string res;
    res.pb(*z.begin());
    for (auto x : tb) res.pb(x.second);
    return res;
  }
