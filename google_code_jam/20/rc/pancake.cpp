...
struct SolverContext {
  ...
  int solve() {
    std::sort(ALL(tb));
    std::unordered_map<std::pair<s64, s64>, std::vector<std::pair<s64, int>> > mp;
    REP (i, n)
      FOR (j, 1, d + 1)
        mp[frac<s64>(tb[i], j).as_pair()].emplace_back(j, i);
    int best = d;
    for (auto x : mp) {
      std::sort(ALL(x.second));
      int cut = 0, rem = d;
      std::set<int> used;
      for (auto &y : x.second) {
        used.insert(y.second);
        if (y.first <= rem)
          cut += y.first - 1, rem -= y.first;
        else {
          cut += rem;
          rem = 0;
        }
      ...
      if (rem > 0) {
        REPV (i, n) {
          auto tmp = frac<s64>(tb[i]) / frac<s64>(x.first);
          if (used.count(i)) continue;
          s64 have = tmp.floor();
          if (have == 0) break;
          s64 take = std::min<s64>(have, rem);
          cut += take;
          rem -= take;
          if (rem == 0) break;
        }
      }
      if (rem == 0) checkmin(best, cut);
    }
