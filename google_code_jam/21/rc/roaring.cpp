...
struct SolverContext {
  ...
  bool le(const std::string &x) {
    if (x.size() < l.size()) return true;
    if (x.size() > l.size()) return false;
    REP (i, x.size()) {
      if (x[i] < l[i]) return true;
      if (x[i] > l[i]) return false;
    }
    return true;
  }
  void update(std::string cnd) {
    if (best.size() == 0) best = cnd;
    if (best.size() < cnd.size()) return;
    if (best.size() > cnd.size() || (best.size() == cnd.size() && cnd < best)) best = cnd;
  }

  std::string add1(std::string x) {
    REPV (i, x.size()) {
      if (x[i] == '9')
        x[i] = '0';
      else {
  ...
  std::string make_roaring(std::string x) {
    std::string res = x;
    do {
      x = add1(x);
      res += x;
  ...
  std::string solve() {
    update(make_roaring("1"));
    REP (i, l.size()) {
      auto x = l.substr(0, i + 1);
      REP (j, 20) {
        update(make_roaring(x));
        x = add1(x);
      }
      ...
      REP (j, x.size())
        x[j] = '9';
      x.back() = '0';
      if (x.size() > 1) x[x.size() - 2] = '0';
      REP (j, 101) {
        if (x[0] != '0') update(make_roaring(x));
        x = add1(x);
      }
    }
    return best;
  }
  std::string solve_dumb() { return {}; }
};
