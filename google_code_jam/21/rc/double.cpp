...
struct SolverContext {
  ...
  int solve(const std::string &s, const std::string &e) {
    int best = inf;
    vi rs = decomp(s);
    vi re = decomp(e);
    if (rs.size() < re.size()) return best;
    if (s == e) return 0;
    if (e == "0") return rs.size();
    if (rs.size() == re.size() && s.back() == '1') return best;
    // if (re.size() > 2 && rs.size() == re.size()){
    //  return best;
    //}

    FOR (i, std::max<int>(0, rs.size() - re.size()), rs.size() + 1) {
      int cnd = i;
      bool bad = false;
      REP (j, re.size()) {
        if (i + j >= rs.size())
          cnd += re[j] + (j > rs.size());
        else if (rs[i + j] == re[j] || (i + j == rs.size() - 1 && rs[i + j] < re[j]))
          cnd += re[j] - rs[i + j];
        else {
          bad = true;
          break;
  ...
  int solve() {
    ...
    if (s == "1" && e == "0") return 1;
    if (e == s) return 0;
    if (s.size() <= e.size() && e == s + std::string(e.size() - s.size(), '0'))
      return e.size() - s.size();
    int best = inf;
    if (s == "0")
      checkmin(best, solve("10", e) + 2);
    else
      checkmin(best, solve(s, e));
    if (s != "0") checkmin(best, solve(s + "0", e) + 1);
    return best;
  }
  ...
  int solve_dumb() {
    ...
    q.push(s0);
    cost[s0] = 0;
    std::function<void(int, int)> push = [&](int x, int c) {
      if (!cost.count(x) || cost[x] > c) {
        cost[x] = c;
        q.push(x);
      }
    };
