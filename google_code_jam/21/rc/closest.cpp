...
struct SolverContext {
  ...
  bool is_valid(int x) { return x >= 1 && x <= K && !nums.count(x); }

  double solve() {
    std::vector<int> cnds;
    std::vector<int> lst(ALL(nums));
    std::vector<pii> intervals;
    REP (i, lst.size()) {
      if (i == 0)
        intervals.emplace_back(1, lst[i] - 1);
      else
        intervals.emplace_back(lst[i - 1] + 1, lst[i] - 1);
      if (i == lst.size() - 1) intervals.emplace_back(lst[i] + 1, K);
    }
    double best = 0;
    REP (i, intervals.size())
      REP (j, i + 1) {
        int li = intervals[i].second - intervals[i].first + 1;
        int lj = intervals[j].second - intervals[j].first + 1;
        bool one_sidei = intervals[i].first == 1 || intervals[i].second == K;
        bool one_sidej = intervals[j].first == 1 || intervals[j].second == K;
        double v = 0;
        if (i == j) {
          v += li;
        } else {
