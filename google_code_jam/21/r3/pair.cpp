...
struct SolverContext {
  ...
  s64 compute(std::deque<int> lst, s64 low, s64 high) {
    assert(lst.size() % 2 == 0);
    while (lst.size() > 0) {
      int lx = lst.front();
      lst.pop_front();
  ...
  s64 solveit(int pos, std::deque<int> &lst, s64 low, s64 high, bool need_dedup = false) {
    if (pos == ND) return need_dedup ? inf : compute(lst, low, high);

    int sz = lst.size();
    s64 best = inf;
    if (cnt[pos] & 1) lst.pb(pos);
    for (int i = 0; i <= cnt[pos]; i += 2) {
      checkmin(best, solveit(pos + 1, lst, low, high, cnt[pos] - i >= 2 ? pos == 0 : need_dedup));
      lst.pb(pos);
      lst.pb(pos);
    }
  ...
  const s64 inf = 1ull << 62;
  s64 tryit(int i1, int i2) {
    ...
    std::deque<int> lst;
    if (n & 1) {
      if (std::min(i1, i2) == 0) return inf;
      REP (i, ND) {
        REP (j, cnt[i])
          lst.pb(i);
      }
      high = 10 * high + lst.front();
      lst.pop_front();
      return compute(lst, low, high);
    } else {
      if (!i1) return inf;
      return solveit(0, lst, low, high, i2 == 0);
    }
  }
  ...
  s64 solve() {
    ...
    REP (i1, 10)
      REP (i2, 10) {
        --cnt[i1];
        --cnt[i2];
        if (cnt[i1] >= 0 && cnt[i2] >= 0) {
          checkmin(res, tryit(i1, i2));
        }
    ...
    FOR (i, 1, (1 << n) - 1) {
      ...
      do {
        ...
        do {
          ...
          int ia = 0, ib = 0;
          for (auto x : la) ia = ia * 10 + x;
          for (auto x : lb) ib = ib * 10 + x;
          int u = std::abs(ia - ib);
          checkmin(best, u);

        } while (std::next_permutation(ALL(lb)));

      } while (std::next_permutation(ALL(la)));
    }
    return best;
  }
};
