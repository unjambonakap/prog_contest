...
struct SolverContext {
  ...
  std::vector<std::vector<int> > solve() {
    ...
    int xlid = 0;
    int ylid = xlid + nx;
    g.setup(ylid + ny, false);

    int s1 = 0, s2 = 0;
    REP (x, nx)
      g.adde(g.S, xlid + x, 0, xl[x]), s1 += xl[x];
    REP (y, ny)
      g.adde(ylid + y, g.T, 0, yl[y]), s2 += yl[y];
    std::unordered_map<pii, int> pos2eid;
    if (s1 != s2) return {};
    REP (x, nx)
      REP (y, ny)
        pos2eid[MP(x, y)] = g.adde(xlid + x, ylid + y, 0, 1);

    int flow = g.find_flow();
    if (flow != s1) return {};

    std::vector<std::vector<int> > res;
    res.resize(nx, vi(ny, 0));
    for (auto kv : pos2eid) res[kv.first.first][kv.first.second] = g.el[kv.second].cap == 0;
    ...
    while (true) {
      ...
      REP (xb, nx)
        REP (yb, ny) {
          if (!res[xb][yb]) continue;

          REP (xa, xb)
            REP (ya, yb) {
              if (res[xa][ya] && !res[xa][yb] && !res[xb][ya]) {
                res[xa][ya] ^= 1;
                res[xb][ya] ^= 1;
                res[xb][yb] ^= 1;
                res[xa][yb] ^= 1;
  ...
  std::vector<std::vector<int> > solve_dumb() { return {}; }
};
