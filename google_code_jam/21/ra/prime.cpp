...
struct SolverContext {
  char buf[11111];
  std::vector<std::pair<int, s64> > tb;
  int n;
  ...
  void read() {
    if (TEST_RAND) {
      ...
      tb.resize(n);
      REP (i, n)
        scanf("%d%Ld", &tb[i].ST, &tb[i].ND);
    }
  }

  bool checkit(int sum, s64 v) {
    std::vector<int> decomp;
    for (auto &x : tb) {
      int cnt = 0;
      while (v % x.ST == 0) v /= x.ST, ++cnt;
      if (cnt > x.ND) return false;
      sum -= cnt * x.ST;
    }
    return sum == 0 && v == 1;
  }
  ...
  s64 solve() {
    s64 tot = 0;
    for (auto &x : tb) tot += x.ST * x.ND;

    FOR (i, 1, 500 * 10) {
      s64 tg = tot - i;
      if (tg == 1) break;
      if (checkit(i, tg)) return tg;
    }
    return 0;
  }
