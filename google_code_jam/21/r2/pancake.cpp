...
struct SolverContext {
  ...
  int go(int a, int b) {
    ...
    REPV (i, lst.size()) {
      int choice = rem - 1;
      rem -= 1 + lst[i];
      res = res * g_precomp.cnk(choice, lst[i]) % mod;
    }
    return res;
  }
  ...
  int solve() {
    ...
    REP (i, n) {
      stk.resize(tb[i] - 1);
      if (stk.size() > 0) next[i] = stk.back();
      stk.pb(n - i - 1);
    }
    std::reverse(ALL(next));
  ...
  int solve_dumb() {
    ...
    do {
      ...
      vi stk;
      REP (i, n) {
        while (stk.size() > 0 && stk.back() < perm[i]) stk.pop_back();
        stk.pb(perm[i]);
        seen.pb(stk.size());
      }
      res += seen == tb;
