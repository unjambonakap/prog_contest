...
struct Precomp {
  std::vector<std::vector<pii> > decomp;
  int sum[maxn];
  ...
  Precomp() {
    ...
    FOR (i, 3, maxn) {
      ...
      for (int j = 2; j * j <= x; ++j) {
        ...
        if (cnt > 0) {
          decomp[i].emplace_back(j, cnt);
        }
      }
      if (x < i && x != 1) {
        decomp[i].emplace_back(x, 1);
      }
    }
  }

  int go(int END, int x, int curv, int target, int pos = 0) {
    if (target == 0) return 0;
    if (target % curv != 0) return INT_MIN;

    if (pos == decomp[END].size()) {
      if (curv == x || curv < 3) return INT_MIN;
      return go(END, curv, curv, target - curv) + 1;
    }

    int best = INT_MIN;
    auto &e = decomp[END][pos];

    s64 v = curv;
    REP (j, e.second + 1) {
      if (END % v != 0) break;
      checkmax(best, go(END, x, v, target, pos + 1));
      v *= e.first;
    }
    return best;
  }
  ...
  int get(int target) {
    int best = 0;
    FOR (i, 3, target + 1)
      checkmax(best, get(i, target));
    return best;
  }

  int get(int x, int target) {
    if (decomp[x].size() == 0) return 1;
    if (gcd(x, target) < 3) return 0;
    if (sum[x] < target) return 0;
    if (target < x) return 0;
    int r = go(x, 1, 1, target - x) + 1;
    return r;
  }
};
Precomp g_precomp;
struct SolverContext {
  ...
  int solve() { return g_precomp.get(n); }
