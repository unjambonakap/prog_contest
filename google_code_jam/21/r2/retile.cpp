...
struct Graph {
  ...
  void adde(int a, int b, int c, int cap, int rev = 0) {
    int id = el.size();
    Edge e;
    e.from = a;
    e.to = b;
...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      readline() >> ny >> nx >> cost_flip >> cost_swap;
      REP (f, 2) {
        REP (i, ny) {
          std::string s;
          readline() >> s;
  ...
  const int dv[5] = { 1, 0, -1, 0, 1 };
  int idx(int x, int y) { return x * ny + y; }
  int is_in(int x, int y) { return x >= 0 && y >= 0 && x < nx && y < ny; }

  int solve() {
    Graph g;
    int n1 = nx * ny * 2;

    const int infcap = 1000;
    g.setup(n1);
    REP (x, nx)
      REP (y, ny) {
        int ia0 = 2 * idx(x, y);
        ...
        int tend = mp[1][x][y];
        if (tstart != tend) {
          g.adde(g.S, ia0 + tstart, 0, 1);
          g.adde(ia0 + tend, g.T, 0, 1);
          g.adde(ia0 + tstart, ia0 + tend, 2 * cost_flip, 1);
        }

        REP (k, 4) {
          int x2 = x + dv[k], y2 = y + dv[k + 1];
          if (!is_in(x2, y2)) continue;
          int ib0 = 2 * idx(x2, y2);
          int ib1 = ib0 + 1;
          g.adde(ia0, ib0, cost_swap, infcap);
          g.adde(ia1, ib1, cost_swap, infcap);
        }
      }
