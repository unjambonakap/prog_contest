...
struct Sim {
  ...
  void finish() {
    int res = 0;
    if (!TEST_RAND) g_reader.readline() >> res;
    g_sim.push(0);
  }
};
...
struct SolverContext {
  ...
  void play(int a, int b) {
    auto res = sim.push(a, b);
    lines.emplace_back(a, b);
    lines.emplace_back(res.first, res.second);
    avail[0].erase(a);
    avail[0].erase(res.first);
    avail[1].erase(b);
    avail[1].erase(res.second);
  }

  int eval_play(int a, int b, bool flip) {
    if (flip) std::swap(a, b);
    int res = lines.size();
    for (auto l : lines) {
      if (a < l.first && b < l.second) --res;
      if (a > l.first && b > l.second) --res;
    }
    return res;
  }
  ...
  void solve() {
    ...
    int best = -1;
    pii bestp;
    auto updatef = [&](int a, int b, bool flip, bool min_sum = false, int nv = -2) {
      nv = nv == -2 ? eval_play(a, b, flip) : nv;
      auto np = flip ? MP(b, a) : MP(a, b);
      if (best < nv || (best == nv && min_sum && a + b < bestp.first + bestp.second)) {
        best = nv;
        bestp = np;
      }
    };
      ...
      REPV (a, SPLITP)
        FOR (b, SPLITP + 1, mx)
          REP (f, 2)
          {
            int na = a, nb = b;
            if (f) std::swap(na, nb);
            if (avail[0].count(na) && avail[1].count(nb)) updatef(na, nb, false);
          }
      play(bestp.first, bestp.second);
    }

    REP (i, g_sim.n - SPLITP - 1) {
      best = -1;
      for (auto a : avail[0])
        for (auto b: avail[1]) 
            updatef(a, b, false, true);
      play(bestp.first, bestp.second);
    }
    sim.finish();
  }
...
int main() {
  ...
  g_sim.read();
  g_precomp.setup();
  char *envx = std::getenv("ROPES_SPLIT_POINT");
  if (envx != nullptr) {
    SPLITP = toInt(envx);
  }
