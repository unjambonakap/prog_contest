...
struct SolverContext {
  ...
  Point<s64> v1;
  Point<s64> v2;
  std::vector<std::array<int, 2> > tb;
  std::vector<Point<s64> > poslist;
  std::set<s64> el;
  ...
  void read() {
    if (TEST_RAND) {
      ...
      g_reader.readline() >> n >> nx >> ny;
      g_reader.readline() >> v1.x >> v1.y >> v2.x >> v2.y;

      tb.resize(n);
      poslist.resize(n);
      REP (i, n)
        g_reader.readline() >> poslist[i].x >> poslist[i].y >> tb[i][0] >> tb[i][1];
    }
  }

  frac<s128> compute(s64 dx1, s64 dx2, frac<s128> dv1, frac<s128> dv2) {
    std::unordered_map<s64, std::vector<pii> > events;

    frac<s128> tot_area{ std::abs(v1.y * v2.x - v1.x * v2.y), 2 };
    REP (i, n) {
      events[poslist[i].x].emplace_back(i, 1);
      events[poslist[i].x + dx1].emplace_back(i, 2);
      events[poslist[i].x + dx2].emplace_back(i, 3);
    }

    frac<s128> tot2;
    for (auto e : tb) tot2.p += e[1];
    tot2 = tot2 * tot_area;
    frac<s128> tot1;
    frac<s128> res = std::abs(tot1 - tot2);
    OPA_DISP0(dv1, dv2);
    OPA_DISP0(tot1, tot2, res);

    s64 last_x = 0;
    frac<s128> active_cnt1 = 0;
    frac<s128> active_cnt2 = 0;
    for (auto x : el) {
      tot1 = tot1 + active_cnt1 * (x - last_x);
      tot2 = tot2 - active_cnt2 * (x - last_x);
      OPA_DISP0(x, tot1, tot2, last_x);
      for (auto e : events[x]) {
        if (e.second == 1){
          active_cnt1 = active_cnt1 + dv1 * tb[e.first][0];
          active_cnt2 = active_cnt2 + dv1 * tb[e.first][1];
        } else if (e.second == 2){
          active_cnt1 = active_cnt1 - dv1 * tb[e.first][0];
          active_cnt2 = active_cnt2 - dv1 * tb[e.first][1];
          active_cnt1 = active_cnt1 + dv2 * tb[e.first][0];
          active_cnt2 = active_cnt2 + dv2 * tb[e.first][1];
      ...
      last_x = x;
      checkmin(res, std::abs(tot1 - tot2));
    }

    OPA_DISP0(res, tot_area);
    return res / tot_area;
  }
  ...
  frac<s128> solve() {
    ...
    for (auto pt : poslist) el.insert(pt.x), el.insert(pt.x + v1.x), el.insert(pt.x + v2.x);
    if (v1.x > v2.x) std::swap(v1, v2);
    if (v1.x == v2.x) {
      best = compute(v1.x, v1.x, frac<s128>(std::abs(v2.y - v1.y), v1.x), 0);
    } else if (v1.x == 0) {

      best = compute(0, v2.x, 0, frac<s128>(std::abs(v1.y-0), v2.x));
    } else {
      auto p0 = Point<s64>();
      auto ip = Segment<s64>(p0, v2).eval(v1.x);
      auto y1 = v1.y * ip.q;
      auto y2 = ip.p;
      OPA_DISP0(y1, y2);

      best = compute(v1.x, v2.x, frac<s128>(std::abs(y2 - y1), v1.x),
                     frac<s128>(std::abs(y2 - y1), v2.x - v1.x));
    }

    return best;
  }

  frac<s128> solve_dumb() { return {}; }
};
