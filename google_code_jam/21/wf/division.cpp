...
struct SolverContext {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      std::string s;
      g_reader.readline() >> s >> D;
      data.n = s.size();
      for (auto x : s) data.s.pb(x - '0');
    }
  }
  ...
  int solve() {
    s64 mulx = 1;
    std::deque<pii> state[2];
    std::unordered_map<int, int> s2[2];
    REP (f, 2) { state[f].emplace_front(0, f == 0); }
    s64 tot1 = 0;
    s64 tot0 = 0;
    s64 target = 0;
    s64 mx = 1;
    ...
    REPV (i, data.n) {
      ...
      REP (f, 2) {
        s64 xm = 1;
        for (auto &e : state[f]) {
          e.first = (e.first + xm * data.s[i]) % D;
          xm = xm * 10 % D;
        }
      }
      /*
         target0 + target_add = target
         target_add = target - target0
         cur * diff10 + target_add = 0(D)
         => cur * diff10 + target -target0 =0(D)

         */

      target = (target + mx * data.s[i]) % D;
      mx = mx * 10 % D;
      if (state[0].size() == lim) {
        REP (f, 2) {
          auto u = state[f].back();
          state[f].pop_back();
          s64 ns = (D + (target - u.first * mulx) % D) % D;
          (f ? tot1 : tot0) += u.second;
          if (u.first % gcdV != 0) continue;
          add(s2[f][ns], u.second);
        }
        tot1 %= mod;
        tot0 %= mod;
        finish0 += s2[0][target] + s2[1][target];
        finish1 += (mod + tot0 - s2[0][target]) % mod;

        miss = (miss + mulx * data.s[i + lim]) % D;
        mulx = mulx * 10 % D;
      }
      ...
      state[1].emplace_front(0, finish1 % mod);
      state[0].emplace_front(0, finish0 % mod);
    }
    s64 res = 0;
    res += state[0][0].second;
    res += state[1][0].second;
    return res % mod;
  }
