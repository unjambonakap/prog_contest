...
struct SolverContext {
  ...
  bool checkit(std::vector<int> have) {
    REPV (i, M) {
      int rem = have[i] - tb[i];
      if (rem < 0) return false;
      if (i - a >= 0) have[i - a] += rem;
      if (i - b >= 0) have[i - b] += rem;
    }
    return true;
  }
  ...
  int solve() {
    ...
    m.resize(M, std::vector<int>(M, 0));
    REP (i, M)
      m[i][i] = 1;
    int maxs = 1e5;
    int mv = M * 22;
    REP (i, M) {
      if (checkit(m[i])) return i;
    }
    FOR (i, M, maxs) {
      vi nl = m[M - a];
      REP (j, M)
        nl[j] = std::min(mv, nl[j] + m[M - b][j]);
      m.pop_front();
      m.push_back(nl);
      if (checkit(nl)) return i;
    }
    return -1;
  }
