...
struct GlobalSim {
  ...
  void read() {
    if (TEST_RAND) {
      ...
      p = 937467793908762347ll;
      p = 860939810732536850ll;
    } else
      g_reader.readline() >> tn >> n >> b >> p;
  }

  void checkit() {
    OPA_DISP0(scores, p, scores - p);
...
struct Precomp {
  ...
  void setup() { end_dump_v = g_sim.b - 3; }

  struct StateHelper {
    State d0, d1, d2, d3;

    std::pair<double, int> compute(int v) {
      std::pair<double, int> best = MP(-1e9, -1);
      checkmax(best, MP(d0.ev, 0));
      checkmax(best, MP(d1.ev + v, 1));
      checkmax(best, MP(d2.ev + 10 * v, 2));
      checkmax(best, MP(d3.ev + 100 * v, 3));
      assert(best.first > -1);
      return best;
    }
  };

  State states[maxn][maxb][maxn][maxn][maxn];
  StateHelper get_helper(int nwait, int curdump, int n1, int n2, int n3) {
    auto d0 = get(nwait, curdump - 1, n1, n2, n3);
    auto d1 = get(nwait, curdump, n1 - 1, n2 + 1, n3);
    auto d2 = get(nwait, curdump, n1, n2 - 1, n3 + 1);
    auto d3 = get(nwait, curdump, n1, n2, n3 - 1);
  ...
  State get(int nwait, int curdump, int n1, int n2, int n3) {

    if (nwait < 0 || curdump < 0 || n1 < 0 || n2 < 0 || n3 < 0) {
      State invalid;
      invalid.ev = -1e9;
      return invalid;
    }
    if (nwait + curdump + n1 + n2 + n3 == 0) return State();

    State &x = states[nwait][curdump][n1][n2][n3];
    if (x.done) return x;
    x.done = true;
    x.ev = 0;
    if (curdump == 0 && nwait > 0)
      x = get(nwait - 1, end_dump_v, n1, n2, n3);
    else {
      auto sh = get_helper(nwait, curdump, n1, n2, n3);
      REP (f, 10) { x.ev += sh.compute(f).first / 10; }
    }
    return x;
  }
};
...
struct Sim {
  std::vector<vi> state;
  Sim() { state.resize(g_sim.n); }
  void gen_rand() {}
  bool check_ok() { return false; }

  int get_block() const {
    if (TEST_RAND) return rng() % 10;
    int x;
    g_reader.readline() >> x;
    return x;
  }
  ...
  void place(int pile, int v) {
    assert(pile >= 0 && pile <= g_sim.n);
    auto &p = state[pile];
    assert(p.size() < g_sim.p);
    p.pb(v);
    if (!TEST_RAND) {
      printf("%d\n", pile + 1);
      fflush(stdout);
...
struct SolverContext {
  ...
  void solve() {
    int rem = g_sim.n * g_sim.b;
    int rem_dump = g_sim.n - 1;
    std::vector<int> rx[4];
    int dumpv = g_precomp.end_dump_v;

    while (rem > 0) {
      --rem;
      int v = sim.get_block();
      auto sh = g_precomp.get_helper(rem_dump, dumpv, rx[0].size(), rx[1].size(), rx[2].size());
      auto r = sh.compute(v);
      int pile = -1;
      if (r.second == 0) {
        --dumpv;
        ...
        if (dumpv == 0) {
          rx[0].pb(rem_dump);
          if (rem_dump > 0) --rem_dump, dumpv = g_precomp.end_dump_v;
        }
      } else {
        pile = rx[r.second - 1].back();
        rx[r.second - 1].pop_back();
        rx[r.second].pb(pile);
      }
      sim.place(pile, v);
    }
  ...
  bool is_ok() const {
    if (TEST_RAND) return sim.check_ok();
    return false;
  }
};
