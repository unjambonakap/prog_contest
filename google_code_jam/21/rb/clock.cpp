...
struct SolverContext {
  ...
  s64 mod = 1ull * int(1e9) * 3600 * 12;
  s64 ns2tick = mod / 60 / int(1e9);
  s128 min_ang = mod / 60;
  s128 hour_ang = mod / 12;
  s64 inv59 = 171335593220339LL;

  std::vector<s64> go(std::vector<s64> x) {
    std::vector<s64> res;
    REPV (i, 3)
      x[i] += (720 - x[0]) % 720;
    auto add1 = (12 - x[1]) % 12;
    for (auto &v : x) v = (v + add1) % mod;
    auto ox = x;
    s64 a = (mod + x[0] - 60 * (x[1] % min_ang)) % min_ang * (s128)inv59 % min_ang;
    for (auto &v : x) v = (mod + v + a) % mod;

    s64 a2 = -(x[0] - 60 * (x[1] % min_ang)) % mod;
    for (auto &v : x) v = (mod + v + a2) % mod;
    std::vector<s64> cnd;
    cnd.pb(x[0] / ns2tick % int(1e9));
    cnd.pb(x[0] / (mod / 60));
    cnd.pb(x[1] / (mod / 60));
    cnd.pb(x[2] / (mod / 12));
    if (x[0] * min_ang != x[1] % min_ang * mod) return res;
    if (x[1] * hour_ang != x[2] % hour_ang * mod) return res;

    res = cnd;
    return res;
  }

  std::vector<s64> solve() {
    std::sort(ALL(tb));
    do {
      auto res = go(tb);
      if (res.size() == 4) {
    ...
    } while (std::next_permutation(ALL(tb)));
    assert(0);

    return {};
  }
  std::vector<s64> solve_dumb() { return {}; }
};
