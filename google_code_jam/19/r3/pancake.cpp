...
struct Entry {
  ...
  Entry(int l, int r) : L(l), R(r) { count = r - l + 1; }

  bool operator<(const Entry &peer) const { return L < peer.L; }
};
...
struct SolverContext {
  ...
  int solve() {
    ...
    REP (i, n)
      order.emplace_back(tb[i], i);
    std::sort(ALL(order));
    std::set<Entry> s;
    s.insert(Entry(-2, -2));
    s.insert(Entry(n + 2, n + 2));
    s64 res = 0;

    for (auto &x : order) {
      int pos = x.ND;
      auto it = s.lower_bound(Entry(pos, pos));
      Entry cur(pos, pos);
      cur.highest = x.ST;
      ...
      Entry nil(0, 0);
      nil.count = 0;
      nil.highest = cur.highest;

      if (prev.R + 1 == pos)
        s.erase(it++), cur.L = prev.L;
      else
        prev = nil, ++it;
      if (next.L == pos + 1)
        s.erase(it), cur.R = next.R;
      else
        next = nil;

      cur.count += prev.count;
      cur.count += next.count;
      res += mul(prev.cost_left, 1 + next.count);
      res += mul(next.cost_right, 1 + prev.count);
      prev.cost_highest = (prev.cost_highest + mul(prev.count, cur.highest - prev.highest)) % mod;
      next.cost_highest = (next.cost_highest + mul(next.count, cur.highest - next.highest)) % mod;
      cur.cost_highest = (prev.cost_highest + next.cost_highest) % mod;

      cur.cost_left =
        (prev.cost_left + next.cost_left + mul(next.cost_highest, prev.count + 1)) % mod;
      cur.cost_right =
        (prev.cost_right + next.cost_right + mul(prev.cost_highest, next.count + 1)) % mod;

      //res += mul(prev.cost_left, next.cost_right);
      OPA_DISP0(x.ST, pos, cur.L, cur.R);
      OPA_DISP0(cur.cost_left, cur.cost_right, cur.cost_highest, res);
  ...
  int solve_dumb() {
    ...
    REP (j, n)
      REP (i, j + 1) {
        int l = -1, r = -1;
        std::unordered_map<int, int> lx, rx;
        FOR (k, i, j + 1)
          checkmax(l, tb[k]), lx[k] = l;
        FORV (k, j, i)
          checkmax(r, tb[k]), rx[k] = r;
        FOR (k, i, j + 1)
          res = (res + std::min(lx[k], rx[k]) - tb[k]) % mod;
      }
    return res;
  }
};
