...
template <typename T> struct IncrementalHull {
  struct PointCompare {
    ...
    bool operator()(const int &a, const int &b) const { return ih.tb[a].x < ih.tb[b].x; }
  };
  std::vector<Point<T> > tb;
  std::set<int, PointCompare> hull;
  bool upper;

  IncrementalHull() : hull(PointCompare(*this)) {}
  ...
  bool push(Point<T> pt) {
    ...
    while (it != hull.end()) {
      ...
      if (!can_remove(id, *it, *next)) break;
      OPA_DISP("Remove 1", id, *it, *next);
      it = hull.erase(it);
    }
    bool insert = false;
...
struct SolverContext {
  ...
  std::vector<int> solve() {
    ...
    {
      DynamicHull dhull_pt1, dhull_pt2;
      dhull_pt1.setup(set1);
      dhull_pt2.setup(set2);
      ...
      REP (i, cnds.size()) {
        ...
        if (1) {
          if (i > 0) {
            auto pt1 =
              dhull_pt1.pts[dhull_pt1.find(dhull_pt1.lowerhull, [&](const Segment<s64> &cur) {
                return s.signed_dist(cur.b) < s.signed_dist(cur.a);
              })];

            auto pt2 =
              dhull_pt2.pts[dhull_pt2.find(dhull_pt2.upperhull, [&](const Segment<s64> &cur) {
                return s.signed_dist(cur.b) > s.signed_dist(cur.a);
              })];
        ...
        dhull_pt1.update(segs[cnds[i]].a, false, dhull_pt1.lowerhull);
        dhull_pt2.update(segs[cnds[i]].b, false, dhull_pt2.upperhull);
      }
    }
    {
      DynamicHull dhull_pt1, dhull_pt2;
      ...
      REPV (i, cnds.size()) {
        ...
        if (1) {
          if (i < cnds.size() - 1) {
            auto pt1 =
              dhull_pt1.pts[dhull_pt1.find(dhull_pt1.upperhull, [&](const Segment<s64> &cur) {
                return s.signed_dist(cur.b) > s.signed_dist(cur.a);
              })];
            auto pt2 =
              dhull_pt2.pts[dhull_pt2.find(dhull_pt2.lowerhull, [&](const Segment<s64> &cur) {
                return s.signed_dist(cur.b) < s.signed_dist(cur.a);
              })];
            if (s.signed_dist(pt1) > 0 || s.signed_dist(pt2) < 0) blacklist.insert(cnds[i]);
          }
        }
        dhull_pt1.update(segs[cnds[i]].a, false, dhull_pt1.upperhull);
        dhull_pt2.update(segs[cnds[i]].b, false, dhull_pt2.lowerhull);
      }
    }
