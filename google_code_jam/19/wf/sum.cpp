...
struct hash<std::pair<T1,T2> > {
  ...
  template<typename T1> 
    struct hash<std::deque<T1> >
    {
    size_t operator()(const std::deque<T1> &x) const {
    size_t h = 0;
    for (auto &e : x) h = h*31 + std::hash<T1>{}(e);
    return h;
  }
    };
  template<typename T1> 
...
struct State {
  std::deque<int> left_target;
  int right_val;
};

struct cur_hash_t {
  size_t operator()(const State &x) const { return std::hash{}(MP(x.left_target, x.right_val)); }
};

struct DP {
  static const int maxc = 2;
  static constexpr int maxs = 3;
  std::unordered_map<State, bool, cur_hash_t> dp[maxp][maxs + 1]; // pos, cndpos
  int m;
  int p[3];
  int tens[3];
  int n;
  ...
  bool go(const State &s, int pos, int right_pos) {
    auto r = dp[pos][right_pos].insert(MP(s, false));
    bool &res = r.first->second;
    if (r.second) return res;
    if (pos < (p[0]+1)/2){ // end

    }
    if (pos > p[right_pos]) return go(s, pos, right_pos+1);
    if (right_pos == m){
      if (s.right_val) return res;
      if (s.left_target[0] > 2) return res;

      State ns = s;
      ns.left_target.pb(0);
      ns.left_target.pop_front();
      ns.left_target[0] += s.left_target[0] * 10;
      return go(ns, pos+1, 0);
    }
...
struct SolverContext {
  ...
  void read() {
    std::string s;
    readline() >> s;
    for (auto &a : s) q.pb(a - '0');
    n = q.size();
  }

  std::vector<vi> solve2() {
    FOR (i, n - 2, n - 1)
      REP (j, i + 1) {}
  }

  std::vector<vi> solve3() {
    int maxd = 2;
    REP (i, maxd + 1)
      REP (j, i + 1)
        REP (k, j + 1) {
          ...
          State s0;
          s0.right_val = 0;
          s0.left_target.pb(q[n - 1]);
          if (dp.go(s0, n - 1, 0)) return dp.res;
        }
    return {}
  }
  ...
  std::vector<vi> solve_dumb() { return {}; }
};
