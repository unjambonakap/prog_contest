...
struct Sim {
  ...
  void push(int v, int token) { states[v].pb(token); }
  std::vector<int> query(int v) { return states[v]; }

  bool is_winning() const {
    std::vector<int> poslist;
    poslist.pb(0);
    FOR (i, 1, states.size()) {
      if (states[i].size() < states[poslist[0]].size()) {
        poslist.clear();
        poslist.pb(i);
      } else if (states[i].size() == states[poslist[0]].size())
        poslist.pb(i);
    }
    int u = poslist[0];
    if (poslist.size() > 1) return false;
    std::set<int> sx(ALL(states[u]));
    if (sx.size() < states[u].size()) return false;
    return sx.count(selfid);
  }
...
struct SolverContext {
  ...
  void read() {}
  std::vector<int> query(int v) {
    OPA_DISP("Queyr ", v);
    if (TEST_RAND) return sim.query(v);
  ...
  bool is_ok() {
    if (TEST_RAND) return sim.is_winning();
    return false;
  }

  void act(int d) {
    int winpots = 4;
    int start_q = ndays - winpots - 1;
    if (d < npots)
      put(d, selfid);
    else if (d == ndays - 1) {
      int best = -1;
      for (auto &kv : sx)
        if (best == -1 || sx[best].size() > kv.second.size()) best = kv.first;
      sx.erase(best);
      best = -1;
      ...
      put(d % (npots - winpots) + winpots, selfid);
...
int main() {
  ...
  OPA_DISP0(ok_count, tn, 1. * ok_count / tn);
