...
struct SolverContext {
  ...
  void read() {
    ...
    REP (i, ny) {
      ...
      OPA_DISP0(buf);
      REP (j, nx)
        mp[j + 1][i + 1] = (buf[j] == '#') + mp[j + 1][i] + mp[j][i + 1] - mp[j][i];
    }
  }

  int query(int xl, int xh, int yl, int yh) {
    return mp[xh + 1][yh + 1] - mp[xl][yh + 1] - mp[xh + 1][yl] + mp[xl][yl];
  }

  int go(int xl, int xh, int yl, int yh, bool first = false) {
    if (xl > xh || yl > yh) return 0;
    int &r = dp[xl][yl][xh][yh];
    if (r != -1) return r;
    std::unordered_set<int> mex;
    int cnt = 0;

    FOR (u, xl, xh + 1) {
      if (query(u, u, yl, yh) > 0) continue;
      OPA_DISP0(u, u, yl, yh, query(u,u,yl, yh));
      int a = go(xl, u - 1, yl, yh);
      int b = go(u + 1, xh, yl, yh);
      int v = a ^ b;
      if (v == 0) cnt += (yh - yl + 1);
      mex.insert(v);
    }
    FOR (u, yl, yh + 1) {
      if (query(xl, xh, u, u) > 0) continue;
      OPA_DISP0(xl, xh, u, u, query(xl, xh, u, u));
      int a = go(xl, xh, yl, u - 1);
      int b = go(xl, xh, u + 1, yh);
      int v = a ^ b;
      if (v == 0) cnt += (xh - xl + 1);
      mex.insert(v);
    }
