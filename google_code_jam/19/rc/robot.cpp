...
std::map<char, int> rmp = { { 'R', 0 }, { 'S', 1 }, { 'P', 2 } };
const std::string mp = "RSP";
const int lim_len = 500;
...
struct SolverContext {
  ...
  void read() {
    ...
    REP (i, n) {
      ...
      tb.emplace_back();
      int sz = strlen(buf);
      REP (j, sz) { tb.back().pb(rmp[buf[j]]); }
    }
  }

  int q(int x, int pos) const {
    const auto &e = tb[x];
    return e[pos % e.size()];
  }
  bool win(int self, int b) {

    REP (j, 2 * lim_len) {
      int aself = q(self, j);
      int apeer = q(b, j);

      if (aself == (apeer + 1) % 3) return false;
      if (aself == (apeer + 2) % 3) return true;
    }
    return false;
  }

  bool dfs(const std::vector<int> &state, int pos) {
    ...
    std::vector<int> split[3];
    for (auto x : state) split[q(x, pos)].pb(x);
    res.pb(0);
    REP (f, 3) {
      res.back() = f;
      if (split[(f + 2) % 3].size() == 0 && dfs(split[f], pos + 1)) return true;
    }
    res.pop_back();
    return false;
  }
  ...
  std::string solve() {
    ...
    REP (i, n)
      s.pb(i);
    if (!dfs(s, 0)) return "IMPOSSIBLE";
    std::string ans;
    for (auto x : res) ans += mp[x];
    return ans;
  }
};
