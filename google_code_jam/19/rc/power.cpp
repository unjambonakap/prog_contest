...
struct SolverContext {
  ...
  bool checkit(const std::string &ans) {
    if (TEST_RAND) {
      return ans == sol;
    }
    return readc() == 'Y';
  ...
  std::string solve() {
    ...
    REP (i, 4) {
      ...
      int mx = 0;
      for (auto &u : mp) checkmax<int>(mx, u.second.size());
      bool fd = false;
      for (auto v : rem) {
        if (mp[v].size() != mx) {
          res.pb(v);
