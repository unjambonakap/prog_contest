...
struct Interval {
  ...
  int len() const { return r + 1 - l; }

  bool should_merge(const Interval &peer) const { return l == peer.r + 1 || r + 1 == peer.l; }

  Interval merge(const Interval &peer) {
    Interval res(std::min(l, peer.l), std::max(r, peer.r));
    res.cnt = cnt + peer.cnt;
    return res;
  }

  bool operator<(const Interval &peer) const { return l < peer.l; }
};

struct IntervalTree {
  std::set<Interval> il;

  Interval add(Interval u) {
    auto itnext = il.lower_bound(u);
    auto itprev = itnext;
    --itprev;
    auto prev = *itprev;
    auto next = *itnext;
    if (u.should_merge(prev)) il.erase(prev), u = prev.merge(u);
    if (u.should_merge(next)) il.erase(next), u = next.merge(u);
    il.insert(u);
    return u;
  }

  const Interval *query(int pos) const {
    const Interval &itx = *--il.lower_bound(Interval(pos + 1, pos + 1));
    if (itx.l <= pos && pos <= itx.r) return &itx;
    return nullptr;
  }
};
...
struct SolverContext {
  ...
  int n, k;
  vi lst[2];
  std::unordered_map<int, std::vector<int> > poslist[2];
  IntervalTree it[2];
  void read() {
    scanf("%d%d", &n, &k);
    REP (f, 2) {
      ...
      REP (i, n) {
        scanf("%d", &lst[f][i]);
        poslist[f][lst[f][i]].pb(i);
      }
    }
  }

  s64 add1(int pos) {
    auto ix = it[0].add(Interval(pos, pos));
    auto ix2 = it[1].query(pos);
    if (ix2 == nullptr) return 0;
    int l = std::max(ix.l, ix2->l);
    int r = std::min(ix.r, ix2->r);
    return 1ll * (pos - l + 1) * (r - pos + 1);
  }

  void add2(int pos) { it[1].add(Interval(pos, pos)); }

  s64 go(int f) {
    int fi = f ^ 1;
    REP (f, 2) {
      it[f] = IntervalTree();
      it[f].il.insert(Interval(-2, -2));
      it[f].il.insert(Interval(n+2, n+2));
    }

    s64 res = 0;
    REP (lf, maxv) {
      int lfi = lf - k;
      if (poslist[f].count(lf)) for (auto &e : poslist[f][lf]) res += add1(e);
      if (poslist[fi].count(lfi)) for (auto &e : poslist[fi][lfi]) add2(e);
    }
    return res;
  }
