...
#define TEST_CHECK 0
#endif
const int dv[] = { 0, 1, 0, -1, 0 };
const char dirmap[] = "NESW";
const int maxn = 555;

struct SolverContext {
  std::vector<pii> pl;
  std::vector<int> dir[4];
  std::unordered_map<int, int> rmp[2];
  std::vector<int> xl, yl;
  int n;
  int q;
  int state[maxn * 2][maxn * 2];
  int nx, ny;
  ...
  void read() {
    ...
    REP (i, n) {
      char c;
      scanf("%d%d %c", &pl[i].ST, &pl[i].ND, &c);
      int d;
      REP (j, 4)
        if (c == dirmap[j]) d = j;
      pii np(pl[i].ST + dv[d], pl[i].ND + dv[d + 1]);
      xl.pb(np.ST);
      yl.pb(np.ND);
      xl.pb(pl[i].ST);
      yl.pb(pl[i].ND);
  ...
  void add(pii pa, pii pb) {
    int xa = std::min(pa.ST, pb.ST);
    int ya = std::min(pa.ND, pb.ND);
    int xb = std::max(pa.ST, pb.ST);
    int yb = std::max(pa.ND, pb.ND);
    OPA_DISP0(pa, pb);
    xa = rmp[0][xa];
    xb = rmp[0][xb];
    ya = rmp[1][ya];
  ...
  pii solve() {
    ...
    REP (d, 4) {
      for (auto &e : dir[d]) {
        pii p(pl[e].ST + dv[d], pl[e].ND + dv[d + 1]);
        pii high(q, q), low(0, 0);
        if (dv[d] == -1)
          high.ST = p.ST;
        else if (dv[d] == 1)
          low.ST = p.ST;
    ...
    REP (i, nx)
      REP (j, ny) { state[i + 1][j + 1] += state[i][j + 1] + state[i + 1][j] - state[i][j]; }

    std::pair<int, pii> res(-1, MP(0, 0));
    REP (i, nx)
      REP (j, ny)
        checkmax(res, MP(state[i + 1][j + 1], MP(-i, -j)));
    OPA_DISP0(res.ST);
    return MP(xl[-res.ND.ST], yl[-res.ND.ND]);
  }
};
