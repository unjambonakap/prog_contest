...
const int maxn = 111;
std::pair<double, double> w0[maxn];
int n;
double p, p0;
double best;

void update(double v) { best = std::max(best, std::min(p, v)); }

double solve() {
  best = 0;
  REP (i, n) {
    if (p >= w0[i].ST) update(w0[i].ND);
  }
  ...
  if (n >= 2) {
    ...
    REP (i, n) {
      REP (j, i) {
        ...
        if (w0[i].ST + w0[j].ST > p) continue;
        double mx = w0[i].ND + w0[j].ND;
...
int main() {
  ...
  REP (ti, tn) {
    ...
    REP (i, n) {
      ...
      if (a > b) std::swap(a, b);
      p -= 2 * (a + b);
      w0[i] = std::pair<double, double>(2 * a, 2 * sqrt(a * a + b * b));
    }
    std::sort(w0, w0 + n, [](auto &a, auto &b) { return a.ND < b.ND; });
