...
struct State {
  ...
  void disp() const { OPA_DISP0(mp[0], mp[1], anys[0], anys[1]); }

  bool valid = false;

  void merge_from(const State &l, const State &r) {
    REP (f, 2)
      mp[f].clear();
    valid = false;
    anys[0] = anys[1] = -1;
    ...
    REP (f, 2) {
      if (l.anys[f] == r.anys[f]) anys[f] = l.anys[f];
    }
    fill(l, r);
    fill(r, l);
    REP (f, 2)
      if (anys[f] != -1) mp[f ^ 1].clear();
    valid = false;
    REP (f, 2)
      valid |= anys[f] != -1 || mp[f].size() > 0;
  }

  void fill(const State &l, const State &r) {

    REP (f, 2)
      if (l.anys[f] != -1 && r.anys[f ^ 1] != -1)
        mp[f][l.anys[f]] = r.anys[f ^ 1];
    ...
    REP (f, 2) {
      ...
      for (auto &x : l.mp[f]) {
        int v1 = x.second;
        int v2;
        if (r.mp[f].count(x.first))
          v2 = r.mp[f].find(x.first)->second;
        else {
          if (r.anys[fi] != -1)
            v2 = r.anys[fi];
          else
...
pii solve() {
  ...
  return MP(result.score, result.count);
}
...
bool can_dumb(int l, int r) {
  std::set<int> vals[2];
  FOR (i, l, r) {
    REP (f, 2)
      if (v[f][i] != v[f][l]) vals[f ^ 1].insert(v[f ^ 1][i]);
  }
  return vals[0].size() <= 1 || vals[1].size() <= 1;
}
...
pii dumb() {
  ...
  REP (i, n)
    FOR (j, i, n + 1)
      if (can_dumb(i, j)) res.update(j - i);
  return MP(res.score, res.count);
}
