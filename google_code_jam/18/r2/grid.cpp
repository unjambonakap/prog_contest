...
bool in(int x, int y, int xm, int ym, int inv, bool twoside) {
  if (twoside) {
    return (x <= xm) ^ (y <= ym) ^ inv ^ 1;
  }
  return (x <= xm && y <= ym) ^ inv;
}

int compute(bool twoside) {
  int res = 0;
  bool hash = false, hasv = false, haselse = false;

  REP (x, nx) {
    REP (y, ny)
      if (mp[x][y] == 0) {
        hash |= (y + 1 < ny && mp[x][y + 1]);
        hasv |= (x + 1 < nx && mp[x + 1][y]);

        if (y + 1 < ny && x + 1 < nx) {

          if (twoside)
            haselse |= mp[x][y + 1] && mp[x + 1][y] && !mp[x + 1][y + 1];
          else
            haselse |= mp[x][y + 1] && mp[x + 1][y] && mp[x + 1][y + 1];
        }
      }
  ...
  REP (xm, nx + 1)
    REP (ym, ny + 1) {
      if (xm < nx && ym == ny && !hasv) continue;
      if (xm == nx && ym < ny && !hash) continue;
      if (xm < nx && ym < ny && !haselse) continue;
      memset(par, -1, sizeof(par));
      REP (x, nx)
        REP (y, ny)
          if (in(x, y, xm, ym, mp[x][y], twoside)) REP (k, 4) {
              int xx = x + dd[k];
              int yy = y + dd[k + 1];
              if (xx < 0 || yy < 0 || xx >= nx || yy >= ny) continue;
              bool is_in = in(xx, yy, xm, ym, mp[xx][yy], twoside);
              if (is_in) join(x * ny + y, xx * ny + yy);
            }
      REP (i, nx * ny)
        if (par[i] < 0) checkmax(res, -par[i]);
    }
  return res;
}
...
int dumb() {
  ...
  REP (m, 1 << (nx * ny)) {
    ...
    REP (x, nx)
      REP (y, ny)
        if (m >> (x * ny + y) & 1) can.emplace(x, y);
    memset(par, -1, sizeof(par));
    for (auto &p : can)
      for (auto &p2 : can) {
        int diff = abs(p.first - p2.first) + abs(p.second - p2.second);
        if (diff == 1) join(p.first * ny + p.second, p2.first * ny + p2.second);
      }

    bool ok = false;
    REP (i, nx * ny)
      if (par[i] == -can.size()) ok = true;
    if (!ok) continue;
    ok = false;
    ...
    REP (xm, nx + 1)
      REP (ym, ny + 1) {
        ...
        bool good = true;
        for (auto &x : can) {
          int quad = (x.first <= xm) + 2 * (x.second <= ym);
          has[quad] |= 1 << mp[x.first][x.second];
        }

        REP (f, 4)
          if (has[f] == 3) good = false;
        ok |= good;
      }
    if (ok) checkmax<int>(res, can.size());
  }
  return res;
}
