...
struct BGraph {
  int na;
  std::vector<std::vector<int>> a2b;
  std::vector<int> peer;
  std::vector<int> vis;
  int visid=0;
  BGraph(int na, int nb): na(na){
    a2b.resize(na);
    vis.resize(na, 0);
    peer.resize(nb, -1);
  ...
  int dfs(int a){
    if (vis[a] == visid) return 0;
    vis[a] = visid;
    for (auto &b : a2b[a]){
      if (peer[b] == -1 || dfs(peer[b]))
      {
        peer[b] = a;
        return 1;
      }
...
int solve() {
  ...
  for (auto &kv : mp) {
    ...
    for (auto &x : kv.second) {
      xmap.insert(MP(x.first, xmap.size()));
      ymap.insert(MP(x.second, ymap.size()));
    }

    BGraph bg(xmap.size(), ymap.size());
    for (auto &x : kv.second){
      bg.a2b[xmap[x.first]].pb(ymap[x.second]);
    }
    int mm = bg.max_match();
    res += kv.second.size()  - mm;
  }
