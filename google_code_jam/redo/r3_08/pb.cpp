...
void bfsa(){
    ...
    REP(i, xn) REP(j, yn) if (mp[i][j]) q.push(MP(i, j)), dist[i][j]=0;
    while(!q.empty()){
        pii p=q.front(); q.pop();
        ...
        REP(k, 4){
            int nx=p.ST+vx[k];
            int ny=p.ND+vy[k];
            if (in(nx, ny) && !mp[nx][ny] && dist[nx][ny]==-1) dist[nx][ny]=c+1, q.push(MP(nx, ny));
        }
    }
}
...
void bfsb(){
    ...
    while(!q.empty()){
        ...
        REP(k, 4){
            ...
            x-=vx[k], y-=vy[k];
            if (!visited[x][y]) q.push(state(x, y, p.c+dist[p.x][p.y]));
        }
