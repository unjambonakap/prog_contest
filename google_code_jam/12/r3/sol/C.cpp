...
int64 solve()
{
	...
	for (int i=0;i<n;i++)
	{
		int64 d=max(0LL,S[i]+1-C);
		checkmin(d,(M-W)/P[i]);
		if (d>0)
		{
			W+=P[i]*d;
			C+=d;
			a.push_back(MP(P[i],d));
		}
	}
	int64 R=0;
	...
	{
		int64 MS=D,C=0;
		for (int i=0;i<=k;i++) MS+=a[i].first*a[i].second,C+=a[i].second;
		if (MS>M) continue;
		checkmax(R,C*(M/MS));
		if (k+1<SIZE(a))
		{
			int64 p=a[k+1].first;
			int64 e=a[k+1].second;
			int64 H=1;
			int64 T=M/MS;
			for (;H-T>100;)
			{
				int64 M1=H+(T-H)/10;
				int64 M2=T-(T-H)/10;
				int64 t1=M1*C+(M-M1*MS)/p;
				if ((M-M1*MS)/p>=M1*e)
				{
					H=M1;
					continue;
				}
				int64 t2=M2*C+(M-M2*MS)/p;
				checkmax(R,t1);
				checkmax(R,t2);
				if (t1>t2) H=M2;
				else H=M1;
			}
			for (int M1=H;M1<=T;M1++)
			{
				int64 t1=M1*C+(M-M1*MS)/p;
				if ((M-M1*MS)/p>=M1*e)
					continue;
...
{
//	__filename="C";
//	__filename="C-small-attempt0";
	__filename="C-small-attempt5";
//	__filename="C-large";
	if (argc==1)
	{
		__mod=1;
