...
map<vector<int>,int> M[10],G[10];

int DFS(vector<int> state)
{
	if (M[n].find(state)!=M[n].end()) return M[n][state];
	int ret=1000000;
	int mask=0;
	for (int i=0;i<n;i++) 
	{
		...
		for (int k=0;k<n;k++)
		{
			if (k==0) continue;
			vector<int> g;
			for (int j=0;j<SIZE(state);j++) if (a[state[j]][i]==k) g.push_back(state[j]);
			if (SIZE(g)==SIZE(state))
			{
				tmp=1000000;
				break;
			}
		...
		tmp++;
		if (tmp<ret) ret=tmp,mask=two(i);
		else if (tmp==ret) mask|=two(i);
		
	}
	G[n][state]=mask;
	return M[n][state]=ret;
}

bool doIt(vector<int> state,int *e,int *p,int left)
{
	if (DFS(state)!=left) return false;
	if (left==1) return true;
	if (!contain(G[n][state],e[0])) return false;
	vector<int> new_state;
	for (int i=0;i<SIZE(state);i++) if (a[state[i]][e[0]]==p[e[0]]) new_state.push_back(state[i]);
	return doIt(new_state,e+1,p,left-1);
}
...
//	freopen("D-large.in","r",stdin);freopen("D-large.out","w",stdout);
	...
	{
		printf("Case #%d:",case_id); //cout<<"Case #"<<case_id<<":";
		scanf("%d",&n);
		m=0;
		int p[8];
		...
		do{
			...
			for (int i=0;i<n;i++) for (int j=i+1;j<n;j++) if (p[i]>p[j]) 
				for (int k=j+1;k<n;k++) if (p[j]>p[k])
					ok=false;
			if (!ok) continue;
			for (int i=0;i<n;i++) a[m][i]=p[i];
			m++;
		}while (next_permutation(p,p+n));
		//printf("m = %d\n",m);
		vector<int> state;
		for (int i=0;i<m;i++) state.push_back(i);
		DFS(state);
		for (int i=0;i<n;i++)
		{
			scanf("%d",&e[i]);
		...
		do{
			...
			for (int i=0;i<n;i++) for (int j=i+1;j<n;j++) if (p[i]>p[j]) 
				for (int k=j+1;k<n;k++) if (p[j]>p[k])
					ok=false;
			if (!ok) continue;
			if (p[e[n-1]]!=0) continue;
			if (doIt(state,e,p,n)) 
				for (int i=0;i<n;i++) ret[i]=p[i];
		}while (next_permutation(p,p+n));
