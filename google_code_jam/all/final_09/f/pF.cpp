...
inline int sgn(long double a){return (a < 0)?-1:(a==0)?0:1;}
...
long double getAngle(long double x, long double y){
	...
	if (x >= 0 && y+EPS < 0)angle=2*PI-angle;
...
bool compareAngle(const Line &a, const Line &b){return (a.angle+EPS < b.angle || (fabs(a.angle-b.angle)<=EPS && a.d <= b.d));}
bool compareDist(const Line a, const Line b){return (a.d <= b.d);}
bool comparePoint(const Point &a, const Point &b){return (a.y+EPS <= b.y || (fabs(a.y-b.y)<=EPS && a.x <= b.x));}
...
Point getCircleInt(Pillar &cen, Line &l, Pillar &p){
	...
	a=1., b=(long double)2.*xa*u+2.*ya*v, c=xa*xa+ya*ya-p.r*p.r;
	//printf("%Lf\n", b);
	assert(b <= 0.);
	d=b*b-4*a*c;
	assert(d+EPS >= 0);
	...
	assert(fabs(sqr(cen.x+us*u-p.x)+sqr(cen.y+us*v-p.y)-sqr(p.r))<=EPS);
...
long double pieSlice(Pillar &d, Point &a, Point &b){
	//printf(">>%Lf %Lf %Lf\n", fabs(sqr(a.x-d.x)+sqr(a.y-d.y)-sqr(d.r)), a.x, a.y);
	//printf(">>%Lf %Lf %Lf\n", fabs(sqr(b.x-d.x)+sqr(b.y-d.y)-sqr(d.r)), b.x, b.y);
	assert(fabs(sqr(a.x-d.x)+sqr(a.y-d.y)-sqr(d.r))<=EPS);
	assert(fabs(sqr(b.x-d.x)+sqr(b.y-d.y)-sqr(d.r))<=EPS);
...
long double polygonArea(vector<Point> pl){
	...
	//if (SZ(pl) > 3)
	//FE(it, l)
	//	printf("<<<<<%Lf %Lf %Lf %Lf => %Lf\n", it->ST.x, it->ST.y, it->ND.x, it->ND.y, getAngle(it->ND.x-it->ST.x, it->ND.y-it->ST.y));

	FE(it, l)
		assert(getAngle(it->ND.x-it->ST.x, it->ND.y-it->ST.y)<=PI);
	a=0;
	REP(i, SZ(l)-1)
		a+=trArea(pl[0], l[i].ND, l[i+1].ND);
	//	printf("get tr %Lf %Lf %Lf %Lf %Lf\n", pl[0].x, pl[0].y, l[i].ND.x, l[i].ND.y, l[i+1].ND.x, l[i+1].ND.y);
...
bool crossPoint(Point a, Point b, Point c, Point d, Point &p){
	...
	if (fabs(a.x-b.x)>EPS && (p.x < min(a.x, b.x) || p.x > max(a.x, b.x)))return false; 
	if (fabs(a.y-b.y)>EPS && (p.y < min(a.y, b.y) || p.y > max(a.y, b.y)))return false;
	if (fabs(c.x-d.x)>EPS && (p.x < min(c.x, d.x) || p.x > max(c.x, d.x)))return false;
	if (fabs(c.y-d.y)>EPS && (p.y < min(c.y, d.y) || p.y > max(c.y, d.y)))return false;
...
//			printf("(%d)%d %d =>%Lf %Lf // %d\n", it->id, it->x, it->y, d, angb, it->r);
			...
			if (angb >= 2*PI)
				angb-=2*PI;
//			printf("(%d)%d %d  =>%Lf %Lf with %Lf\n", it->id, it->x, it->y, d, angb, anga);
...
//				printf("laa\n");
			...
			Pillar np=lp[it.pid];
			if (it.pos == 0){
				if (SZ(lopen) == 0 || lopen.front().d >= sqrt(sqr(it.d)-sqr(np.r))){
					nla.pb(it);
				}
				
				//printf("open %d %Lf\n", it.pid, it.angle);
				it.d=sqrt(sqr(it.d)-sqr(np.r));
			   	lopen.pb(it);
				...
				if (lopen.front().pid == it.pid){
					...
					//printf("close %d => %d with %Lf (%d %d now %d\n", it.pid, SZ(lopen), it.angle, np.x, np.y, it.pid);
		...
		REP(j, SZ(la[i])){
			...
			if (ta.pid == -1){
				...
				arl[i].pb(Area(npa, npb, (Pillar){-1, -1, -1, -1}));
				...
				//	printf("pillar %d>>>>>>>>>%Lf %Lf %Lf %Lf => %Lf\n", np.id, npa.x, npa.y, npb.x, npb.y, ar);
...
//				printf("%Lf\n", ar);
				...
				if (pie){
					...
					assert(pieSlice(it->d, ppa, ppb)+EPS >= trArea(ppa, ppb, it->d));
				}
				...
				//printf("%Lf\n", ar);
				//tp.clear();
				//tp.pb((Point){28.378755,13.825547});
				//tp.pb((Point){33.041876, 0.000000});
				//tp.pb((Point){89.567361, 0.000000});
				//tp.pb((Point){61.592545,30.006624});
				//printf("la %Lf %Lf\n", tp[1].x, tp[1].y);
				//ar=polygonArea(tp);
				//printf("%Lf\n", ar);
				//exit(0);
