...
int is_stable(vector<pii> &v){
	...
	REP(i, SZ(v)){
		REP(j, SZ(v)){
			if (i == j)continue;
			if (MDIST(v[i].ST, v[i].ND, v[j].ST, v[j].ND) == 1)
				break;
...
void doit(pii p, int s, int c){
	...
	*u=c;
	if (ep == p && s == es){found=c; return;}

	if (found!= -1 && c >= found)return;
	...
	REP(i, SZ(b)){
		...
		REP(j, 4){
			...
			if (tmap[nx][ny] == EMPTY && tmap[px][py] == EMPTY){
				...
				doit(get_corner(ntemp), get_state(ntemp), c+1);
