...
Point Point::vrotate(double ang)const{ return Point(cos(ang)*x-sin(ang)*y, sin(ang)*x+cos(ang)*y); }
Point& Point::svrotate(double ang){
	double nx, ny;
	nx=cos(ang)*x-sin(ang)*y, ny=sin(ang)*x+cos(ang)*y;
	x=nx, y=ny;
	return *this;
}
Point Point::rotate(const Point& p, double ang)const{
	Point v=*this-p;
	v.svrotate(ang);
	return p+v;
}
Point& Point::srotate(const Point& p, double ang){
	Point v=*this-p;
	v.svrotate(ang);
	x=p.x+v.x, y=p.y+v.y;
	return *this;
...
Line Line::rotate(const Point& p, double ang)const{ return Line::FromVect(vect().vrotate(ang), getPoint().rotate(p, ang)); }

Line& Line::srotate(const Point& p, double ang){
	Line r=rotate(p, ang);
...
double Point::angle()const{
	double r=(fabs(x)<EPS)?PI/2:atan(fabs(y/x));
...
double sliceArea(Point pa, Point pb, const Point& c, bool addPi){
	double s=0., ang;
	//cout<<pa<<pb<<endl;
	pa-=c, pb-=c;
	//printf("%lf %lf\n", pa.angle(), pb.angle());
