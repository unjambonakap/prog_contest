...
struct Point{
	...
	double dist(const Point& p)const{ return sqrt(SQR(p.x-x)+SQR(p.y-y)); }
	...
	Point vrotate(double)const;
	Point& svrotate(double);
	Point rotate(const Point& p, double ang)const;//every rotation are performed counterclockwise
	Point& srotate(const Point& p, double ang);
	Line dual()const;
...
struct Line{
	...
	static Line Vert(double a){return Line(a, 0, true);}
	...
	double eval(double x)const{ assert(!vert);return a*x+b; }
	Point vect()const;
	...
	Line rotate(const Point& p, double ang)const;
	Line& srotate(const Point& p, double ang);
	bool operator<(const Line& l)const{return angle()<l.angle();}

	double a, b;//y=ax+b
	bool vert;//x=a
	int side;
	static double x_eval;
};
...
double polygonArea(const vector<Point>& pl);
double trArea(Point pa, Point pb, const Point& pc);
double sliceArea(Point pa, Point pb, const Point& c, bool addPi=false);
