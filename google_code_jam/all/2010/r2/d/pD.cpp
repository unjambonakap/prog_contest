...
long double solve(int tt){
	int i, j, k, id;
	long double a, area;
	Point p, np;
	Line l;
	long double aint[4], cint[4], r;
	vector<Line> sl;
	vector<Point> dpl;
	Point pa, pb, pc;
...
//			cout<<pa<<endl;
		...
		REP(i, SZ(hull)-1){
			...
			pa=pl[hull[i].data], pb=pl[hull[i+1].data];



			vector<Point> inter=circleIntersection(pa, pb, pa.dist(ql[tt]), pb.dist(ql[tt]));
			assert(SZ(inter)==2);
			fList.pb((inter[0].dist(ql[tt])>inter[1].dist(ql[tt]))?inter[0]:inter[1]);
