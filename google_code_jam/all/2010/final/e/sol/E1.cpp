...
int simulate(int p,double x0,double y0,double L)
{
	double direction=atan2(y0-y[p],x0-x[p]);
	double min_pos=1e100,max_d=0;
	int next=-1;
	for (int i=0;i<n;i++) if (i!=p)
	{
		double d=sqrt(sqr(x[i]-x[p])+sqr(y[i]-y[p]));
		if (d>L-eps)
			continue;
		double ps=atan2(y[i]-y[p],x[i]-x[p]);
		ps-=direction;
		while (ps<eps) ps+=pi*2.0;
		while (ps>pi+pi+eps) ps-=pi*2.0;
		if (ps<min_pos-eps)
		{
			min_pos=ps;
			max_d=d;
	...
	if (next<0)
		return 0;
	double ps=atan2(y[next]-y[p],x[next]-x[p]);
	return 1+simulate(next,x[p]+L*cos(ps),y[p]+L*sin(ps),L-max_d);
}
int main(int argc,char **args)
{
//	printf("Hello World!\n");
//	freopen("E.in","r",stdin);
//	freopen("E-small-attempt0.in","r",stdin);freopen("E-small-attempt0.out","w",stdout);
//	freopen("E-small-attempt1.in","r",stdin);freopen("E-small-attempt1.out","w",stdout);
//	freopen("E-small-attempt2.in","r",stdin);freopen("E-small-attempt2.out","w",stdout);
